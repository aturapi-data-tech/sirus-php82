...
| PATCH batal Task → **HTTP 200** berisi OperationOutcome `duplicate … sisrute_operation=send_task sisrute_status=409` | Bug pusat (23/09/26): FHIR sudah `cancelled`, tapi pembatalan diteruskan ke SISRUTE sebagai CREATE → tugas di kotak masuk RS tujuan bisa tetap `requested`. Sekelas dengan issue tracker #6 (ServiceRequest → `CarePlan dengan id … sudah ada` 409, "sedang diperbaiki") & #11 (Task baru → `patch_task` 404): lapisan penerus FHIR→SISRUTE salah memilih create/update. Bukan payload kita: log `web_log_status` → batal 27–28/08 BERSIH (balas Task `cancelled`), sejak 22/09 22:19 SETIAP batal kena ini, baik Task dari bundle Task+CarePlan maupun Task-saja — muncul bersamaan dengan penerus SISRUTE baru (22/09) | `batalkanTugas()` tetap membersihkan id + toast KUNING minta kabari RS tujuan; lapor ke kanal kendala. Hindari kirim-batal berulang saat uji |
