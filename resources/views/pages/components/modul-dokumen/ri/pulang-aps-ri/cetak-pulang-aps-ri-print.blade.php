{{-- resources/views/pages/components/modul-dokumen/ri/pulang-aps-ri/cetak-pulang-aps-ri-print.blade.php --}}
...
<x-pdf.layout-a4-with-out-background title="SURAT PERNYATAAN PULANG ATAS PERMINTAAN SENDIRI">
    ...
    @php
        ...
        // Teks klausa per-versi (SUMBER TUNGGAL: App\Support\Clause\PulangApsClause; fallback v1 utk record legacy)
        $clause = PulangApsClause::get($form['clauseVersion'] ?? 'v1');
    @endphp
    ...
    {{-- ── PERNYATAAN PULANG APS (butir 1) ── --}}
    <table class="w-full text-[11px] border-collapse mb-2" cellpadding="0" cellspacing="0">
        <tr>
            <td class="align-top" style="width:16px">1.</td>
            <td class="align-top">{{ $clause['statementPre'] }}</td>
        </tr>
    </table>
    ...
    <table class="w-full text-[10px] border-collapse mb-2">
        <tr>
            <td class="px-1 py-0.5 w-1/3 align-top">Alasan Pulang</td>
            <td class="px-1 py-0.5 w-2 align-top">:</td>
            <td class="px-1 py-0.5 font-bold">{{ ($form['alasanPulang'] ?? '') ?: '-' }}</td>
        </tr>
        @if (!empty($form['keterangan']))
            <tr>
                <td class="px-1 py-0.5 align-top">Keterangan Tambahan</td>
                <td class="px-1 py-0.5 align-top">:</td>
                <td class="px-1 py-0.5">{{ $form['keterangan'] }}</td>
            </tr>
        @endif
    ...
    {{-- ── KLAUSUL PENJELASAN & TANGGUNG JAWAB (butir 2-4) ── --}}
    <table class="w-full text-[11px] border-collapse mb-2" cellpadding="0" cellspacing="0">
        <tr>
            <td class="align-top pb-1" style="width:16px">2.</td>
            <td class="align-top pb-1">{{ $clause['penjelasanRisiko'] }}</td>
        </tr>
        <tr>
            <td class="align-top pb-1" style="width:16px">3.</td>
            <td class="align-top pb-1">{{ $clause['tanggungJawab'] }}</td>
        </tr>
        <tr>
            <td class="align-top" style="width:16px">4.</td>
            <td class="align-top">{{ $clause['kontrolUlang'] }}</td>
        </tr>
    </table>
