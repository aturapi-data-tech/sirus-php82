<?php
// Viewer read-only "Pelaporan Efek Samping Obat (RM 37)" — display Rekam Medis UGD.
// Pembeda entri = id (uuid). Payload cetak bespoke (entry + opsiLabel), jadi tidak
// memakai streamCetakDokumenUgd generik — bentuknya harus persis sama dengan aksi
...
new class extends Component {
    ...
    private string $printView = 'pages.components.modul-dokumen.ugd.pelaporan-eso.cetak-pelaporan-eso-print';
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-pelaporan-eso-ugd-{$this->rjNo}");
    }
    ...
    public function cetak(string $id): mixed
    {
        ...
        return response()->streamDownload(fn() => print $pdf->output(), 'pelaporan-eso-ugd-' . ($data['regNo'] ?? $this->rjNo) . '.pdf');
    }
...
<div>
    ...
    <x-rm.dokumen-view-modal name="view-pelaporan-eso-ugd-{{ $rjNo }}"
