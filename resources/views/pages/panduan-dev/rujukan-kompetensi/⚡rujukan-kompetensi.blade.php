        ...
        { "type": { "coding": [ { "system": "http://terminology.kemkes.go.id",
          ...
          "valueCoding": { "system": "http://hl7.org/fhir/task-status",
                           "code": "accepted", "display": "Accepted" } }
                                ...
                                <p class="ds-body-sm mb-1">Sesudah <em>accepted</em>, perujuk melanjutkan ke ServiceRequest. <strong>Pendaftaran kunjungan pasien rujukan di sisi RS tujuan</strong> (Postman "Faskes Rujukan - Pendaftaran Kunjungan Rujukan") tetap langkah terpisah — dan memang harus terpisah: menyetujui bukan berarti pasien datang.</p>
                                <p class="ds-body-sm">Yang disimpan persetujuan cuma <em>janji</em>-nya (<code>RSTXN_RUJUKANMASUKS</code>). Saat pasiennya tiba, tombol <strong>Rujukan Masuk</strong> di toolbar <code>/ugd/daftar</code> membuka daftar janji yang ditunggu; memilih satu baris membuka form Pendaftaran UGD yang sudah terisi, dan janjinya ditandai terpakai setelah kunjungannya tersimpan. Pencocokan pasien hanya lewat <code>PATIENT_UUID</code> — tidak ketemu itu wajar (baru 4,7% terisi), petugas mencari manual dan IHS-nya ditulis balik saat itu.</p>
                                <p class="ds-body-sm">Rujukan resminya (ServiceRequest) dicari sendiri <strong>tepat sebelum Encounter dibuat</strong> — itu saat paling akhir yang masih berguna untuk mengisi <code>Encounter.basedOn</code>, sekaligus peluang terbesar rujukannya sudah terbit. Yang diterima hanya ServiceRequest yang menunjuk CarePlan atau Task permintaan ini; kecocokan lemah (pasien sama) ditolak, karena salah tempel berarti kunjungan kita tersambung ke rujukan orang lain. Belum terbit → Encounter tetap dikirim tanpa <code>basedOn</code>.</p>
                            </div>
                        </div>
                    </section>
