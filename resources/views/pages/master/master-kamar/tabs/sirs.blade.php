...
<div x-show="tab === 'sirs'" class="flex flex-col h-full">
    ...
    <div class="flex items-center justify-end px-5 py-3 border-b border-gray-100 dark:border-gray-800 shrink-0">
        <x-secondary-button wire:click="loadSirs" wire:loading.attr="disabled"
            wire:target="loadSirs,hapusSirs" class="!py-1 !px-3 !text-xs">
            <x-loading size="xs" wire:loading wire:target="loadSirs,hapusSirs" class="mr-1" />
            <svg wire:loading.remove wire:target="loadSirs,hapusSirs" class="w-3 h-3 mr-1"
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($idTTt !== '')
                                    <x-ghost-button
                                        ...
                                        <x-loading size="xs" wire:loading wire:target="hapusSirs('{{ $idTTt }}')" />
