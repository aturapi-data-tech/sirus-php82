...
new class extends Component {
    ...
    public function rows(): array
    {
        ...
        // Enrich dengan BOR/BTO/TOI — pembaginya hari yang SUDAH berjalan di tiap bulan
        // (bulan berjalan = s/d hari ini, bulan mendatang = 0), bukan panjang kalender penuh.
        $tahun = $this->filterTahun;
        return $this->enrichWithBORTOIBTO($result, $this->kapasitasTT, function ($row) use ($tahun) {
            $awalBulan = Carbon::create($tahun, (int) $row['periode_short'], 1);
            return $this->hariPeriodeBerjalan($awalBulan, $awalBulan->copy()->endOfMonth());
        });
    }
    ...
    /** Panjang kalender penuh tahun terpilih — pembanding hari yang sudah berjalan. */
    public function hariKalender(): int
    {
        return Carbon::create($this->filterTahun, 1, 1)->isLeapYear() ? 366 : 365;
    }
...
<div>
    ...
    <x-kunjungan-ri.rumus :totals="$tot" :kapasitasTT="$kapasitasTT" :hariKalender="$this->hariKalender()" />
    ...
    <x-kunjungan-ri.tabel-bangsal :rows="$this->bangsalBreakdown" :hariPeriode="$tot['days_total']"
        keteranganPeriode="tahun {{ $filterTahun }}" />
