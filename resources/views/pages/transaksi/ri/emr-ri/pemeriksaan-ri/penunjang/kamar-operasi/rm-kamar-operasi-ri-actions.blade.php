...
<div>
    ...
    {{-- MODAL ORDER --}}
    <x-modal name="kamar-operasi-order-ri" size="3xl" focusable>
        <div wire:key="{{ $this->renderKey('kamar-operasi-order-modal-ri', [$riHdrNo ?: 'kosong']) }}">
            ...
            <div class="px-6 py-4 space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <livewire:lov.dokter.lov-dokter target="order-ok-ri-operator" label="Dokter Operator"
                            :initialDrId="$drId ?: null" wire:key="order-ok-operator-{{ $riHdrNo }}-{{ $drId }}" />
                    ...
                    <div>
                        <livewire:lov.dokter.lov-dokter target="order-ok-ri-anestesi" label="Dokter Anestesi"
                            :initialDrId="$drIdOk ?: null" wire:key="order-ok-anestesi-{{ $riHdrNo }}-{{ $drIdOk }}" />
                ...
                <div>
                    <livewire:lov.diagnosa.lov-diagnosa target="order-ok-ri-diagnosa" label="Diagnosa Pra-Operasi (opsional)"
                        :initialDiagnosaId="$diagId" wire:key="order-ok-diagnosa-{{ $riHdrNo }}-{{ $diagId }}" />
                </div>

                {{-- RENCANA TINDAKAN --}}
                <div class="p-3 border rounded-xl border-hairline dark:border-gray-700 bg-surface-soft dark:bg-gray-800/40">
                    <h3 class="mb-2 text-sm font-semibold text-body dark:text-gray-300">Rencana Tindakan (opsional)</h3>

                    <livewire:lov.jasa-dokter.lov-jasa-dokter-ri target="order-ok-ri-tindakan" label="Cari Tindakan Operasi"
                        :riHdrNo="(int) $riHdrNo" wire:key="order-ok-tindakan-{{ $riHdrNo }}-{{ count($selectedTindakan) }}" />
