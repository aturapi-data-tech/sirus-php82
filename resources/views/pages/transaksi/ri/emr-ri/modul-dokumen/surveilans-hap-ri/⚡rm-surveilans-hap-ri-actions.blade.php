...
<div>
    ...
    <x-modul-dokumen.kartu judul="Surveilans Pneumonia Non-Ventilator (HAP)"
        :jumlah="$jumlahEntri"
        satuan="entri"
        :nonaktif="$disabled || !$riHdrNo">
        <x-slot:deskripsi>Pemantauan pneumonia yang timbul selama perawatan TANPA ventilator — lama tirah baring, demam, leukosit, onset baru sputum purulen, serta kultur sputum. Diisi IPCLN / Perawat ruangan.</x-slot:deskripsi>
    ...
    <x-modal name="rm-surveilans-hap-ri-{{ $riHdrNo }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Surveilans Pneumonia Non-Ventilator (HAP)"
                                    ...
                                    <tbody wire:key="surv-entri-{{ $rowKey ?: $loop->index }}" x-data="{ open: false }"
                                        ...
                                        <tr @click="open = !open"
                                            ...
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                ...
                                                <span class="text-muted-soft">·</span> Sputum purulen: {{ $entri['sputumPurulen'] ?: '-' }}
                                            </td>
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                {{ $entri['dokterMerawat'] ?: '-' }}
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$rowFinal" :terkunci="$isFormLocked"
                                                    ...
                                                    judulBukaKunci="Buka Kunci Surveilans HAP"
                                        ...
                                        <tr x-show="open" x-cloak>
                                            <td colspan="7" class="px-4 py-4 bg-surface-soft/60 dark:bg-gray-950/30">
                                                <dl class="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                                                    ...
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Tirah Baring</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ collect([$entri['tglMulaiTirahBaring'] ?? null, $entri['tglSelesaiTirahBaring'] ?? null])->filter(fn($bagian) => filled($bagian))->implode(' s/d ') ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                    ...
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Leukosit</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['leukosit'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Sputum Purulen</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['sputumPurulen'] ?: '-' }}@if (filled($entri['sputumKeterangan'] ?? null)) <span class="text-muted">({{ $entri['sputumKeterangan'] }})</span>@endif
                                                        </dd>
                                                    </div>
