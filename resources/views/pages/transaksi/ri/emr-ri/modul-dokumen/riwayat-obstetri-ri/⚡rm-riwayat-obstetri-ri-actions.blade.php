        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    <h3 class="truncate shrink-0 text-base font-semibold text-ink dark:text-gray-200">Riwayat Obstetri</h3>
                    @if ($roCount > 0)
                        <x-badge class="shrink-0 whitespace-nowrap" variant="success">{{ $roCount }} entri</x-badge>
                    @else
                        <x-badge class="shrink-0 whitespace-nowrap" variant="warning">Belum ada</x-badge>
                    @endif
                    <x-deskripsi-ringkas class="hidden sm:flex text-sm">Riwayat kehamilan/persalinan yang lalu (RM 44.b) — tabel per kehamilan: cara persalinan, tempat, penolong, komplikasi, keadaan &amp; keterangan anak. Header G-P-A. Diisi Bidan/Dokter.</x-deskripsi-ringkas>
                </div>
            </div>
                ...
                <div class="flex items-center gap-3 min-w-0 flex-1 min-w-0">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="flex items-baseline gap-2 min-w-0">
                            <h2 class="text-sm truncate shrink-0 font-semibold text-ink dark:text-gray-100">Riwayat Obstetri</h2>
                            <p class="flex-1 min-w-0 truncate text-xs text-muted dark:text-gray-400">RM 44.b — riwayat kehamilan lalu (VK). Diisi Bidan / Dokter.</p>
                        </div>
                    </div>
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($newForm['rows'] ?? [] as $nomor => $baris)
                                                <tr wire:key="ro-row-{{ $nomor }}">
                                                    ...
                                                    <td class="ds-c">
                                                        @if (!$formReadOnly)
                                                            <x-hapus-button :action="'hapusBaris(' . $nomor . ')'" title="Hapus Baris" :message="'Yakin hapus riwayat kehamilan ke-' . ($nomor + 1) . ' dari daftar?'" />
                                                        @else
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                    <div class="flex flex-wrap items-center justify-center gap-1.5">
                                                        ...
                                                        @endif
                                                        <x-cetak-button wire:click="cetak('{{ $rowKey }}')" title="Cetak entri ini" />
                                                        </div>
                                                        @if (!$isFormLocked)
                                                            <div class="flex items-center justify-center gap-2">
                                                            @if ($isFinal)
                                                            ...
                                                            @endif
                                                            @can('dokumen.hapus')
                                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus entri riwayat obstetri ini?" />
                                                            @endcan
                                                            </div>
                                                        @endif
                                                    </div>
