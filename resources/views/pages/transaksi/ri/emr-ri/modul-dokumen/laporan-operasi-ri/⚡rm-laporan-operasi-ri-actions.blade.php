    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $list[$index]['operatorTtd'] = '';
                $list[$index]['operatorTtdCode'] = '';
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Buka kunci Laporan Operasi (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut, entri kembali draft', 'MR');
            });
            if ($this->editingKey === $createdAt) {
                $this->cancelEdit();
            }
...
<div>
    ...
    <x-modal name="rm-laporan-operasi-ri-{{ $riHdrNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            ...
            <x-modul-dokumen.header judul="Laporan Operasi"
                ...
                jalur="RI" :jumlah="count($laporanList)" :readOnly="$isFormLocked">
                                    ...
                                    <table class="min-w-full text-base">
                                        <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                            <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                                ...
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Tgl Operasi</th>
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Tindakan</th>
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Operator (TTD)</th>
                                                ...
                                                <tr @click="open = !open"
                                                    ...
                                                    <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                        <div class="flex items-center justify-end gap-2">
                                                            ...
                                                            @endif
                                                            <x-cetak-button wire:click="cetak('{{ $rowKey }}')" title="Cetak laporan operasi ini" />
                                                            </div>
                                                            @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                                ...
                                                                @if ($isFinal)
                                                                    @can('dokumen.bukaKunci')
                                                                        <x-confirm-button variant="warning-soft" action="bukaKunci('{{ $rowKey }}')" title="Buka Kunci Laporan Operasi"
                                                                ...
                                                                @endif
                                                                @can('dokumen.hapus')
                                                                <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus laporan operasi ini?" />
                                                                @endcan
                                                                </div>
                                                            @endif
                                                        </div>
