...
new class extends Component {
    ...
    /* ===============================
     ...
     =============================== */
    #[On('lov.selected.surveilans-ilo-room')]
    ...
    #[On('lov.selected.surveilans-ilo-dokter')]
    ...
    public function cetak(string $key)
    {
        ...
        try {
            ...
            set_time_limit(300);
            $pdf = Pdf::loadView('pages.components.modul-dokumen.r-i.surveilans-ilo-ri.cetak-surveilans-ilo-ri-print', ['data' => $data])->setPaper('A4');
                ...
                <div class="w-full space-y-4">
                    ...
                    <fieldset @disabled($formRO) class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.operasi" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['operasi'] ?? null) ? $newForm['operasi'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                ...
                                @unless ($formRO)
                                    <div class="p-3 mt-3 border border-dashed rounded-lg border-gray-300 dark:border-gray-600 bg-canvas dark:bg-gray-800/50">
                                        ...
                                        <div class="grid items-end grid-cols-1 gap-3 lg:grid-cols-5">
                                            <div>
                                                <livewire:lov.room.lov-room target="surveilans-ilo-room" label="Ruang"
                                                    placeholder="Ketik nama ruangan / bed..." :initialRoomId="$barisRawat['roomId'] ?: null"
                                                    wire:key="lov-room-surveilans-ilo-{{ $riHdrNo }}-{{ $renderVersions['modal-surveilans-ilo-ri'] ?? 0 }}" />
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                <livewire:lov.dokter.lov-dokter target="surveilans-ilo-dokter" label="Dokter yang Merawat"
                                                    placeholder="Ketik nama/kode dokter..." :initialDrId="$barisRawat['drId'] ?: null"
                                                    wire:key="lov-dokter-surveilans-ilo-{{ $riHdrNo }}-{{ $renderVersions['modal-surveilans-ilo-ri'] ?? 0 }}" />
                                            </div>
                                            <div>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                <div>
                                    ...
                                    <div class="flex gap-1 mt-1">
                                        ...
                                        <x-now-button wire:click="setNow('tanggalOperasi')" :disabled="$formRO" />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.emergensi" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['emergensi'] ?? null) ? $newForm['emergensi'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.anestesiUmum" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['anestesiUmum'] ?? null) ? $newForm['anestesiUmum'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.implan" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['implan'] ?? null) ? $newForm['implan'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.trauma" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['trauma'] ?? null) ? $newForm['trauma'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.pendekatanEndoskopi" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['pendekatanEndoskopi'] ?? null) ? $newForm['pendekatanEndoskopi'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <x-toggle wire:model="newForm.prosedurMultipel" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['prosedurMultipel'] ?? null) ? $newForm['prosedurMultipel'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                </div>
                                    ...
                                    <thead>
                                        <tr class="bg-surface-soft dark:bg-gray-800">
                                            ...
                                            @for ($h = 0; $h < count($newForm['pemantauan'] ?? []); $h++)
                                                <th class="px-2 py-2 text-center border border-hairline dark:border-gray-700 text-muted">{{ $h + 1 }}</th>
                                            @endfor
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($opsiParamPemantauan as $param => $labelParam)
                                            <tr wire:key="param-{{ $param }}">
                                                ...
                                                @foreach ($newForm['pemantauan'] ?? [] as $h => $hari)
                                                    <td class="px-2 py-2 text-center border border-hairline dark:border-gray-700">
                                                        <x-toggle wire:key="pantau-{{ $h }}-{{ $param }}"
                                                            wire:model="newForm.pemantauan.{{ $h }}.{{ $param }}"
                                                            :current="(bool) ($newForm['pemantauan'][$h][$param] ?? false)"
                                                            :disabled="$formRO" class="justify-center" :trueValue="true" :falseValue="false" />
                                                    </td>
                                                @endforeach
                                            </tr>
                                        @endforeach
                        ...
                        <x-border-form title="5. Hasil Kultur" :collapsible="true" :open="false">
                            <div class="space-y-3">
                                <div class="flex items-center gap-3">
                                    <x-input-label value="Kultur Dilakukan" class="shrink-0" />
                                    <x-toggle wire:model="newForm.kultur" trueValue="Ya" falseValue="Tidak"
                                        :label="filled($newForm['kultur'] ?? null) ? $newForm['kultur'] : 'Belum diisi'" :disabled="$formRO" />
                                </div>
                                <x-surveilans.kultur-list list="kulturHasil" title="Hasil Kultur"
                                    :rows="$newForm['kulturHasil'] ?? []" :baris="$barisKultur['kulturHasil'] ?? []"
                                    :formRO="$formRO" hasilLabel="Hasil" hasilPlaceholder="Hasil kultur"
