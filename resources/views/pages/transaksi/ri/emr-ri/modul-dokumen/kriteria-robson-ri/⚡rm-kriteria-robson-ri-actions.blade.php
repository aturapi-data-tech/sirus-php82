<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/kriteria-robson-ri/rm-kriteria-robson-ri-actions.blade.php
// Dokumen VK/Kebidanan — Kriteria Robson (Klasifikasi 10 Kelompok, WHO 2017).
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-kriteria-robson-ri'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'kriteriaRobsonRI';
    ...
    public function mount(?string $riHdrNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-kriteria-robson-ri']);
        $this->resetNewForm();
    ...
    public function openModal(): void
    {
        ...
        $this->incrementVersion('modal-kriteria-robson-ri');
        $this->layar = 'daftar';
        $this->dispatch('open-modal', name: 'kriteria-robson-ri');
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: 'kriteria-robson-ri');
    }
    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Buka kunci Kriteria Robson (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut', 'MR');
            });
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Hapus Kriteria Robson — ' . $createdAt, 'MR');
            });
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'kriteria-robson-' . ($pasien['regNo'] ?? $this->riHdrNo) . '.pdf');
        } catch (\Throwable $e) {
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="kriteria-robson-ri" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
             wire:key="{{ $this->renderKey('modal-kriteria-robson-ri', [$riHdrNo ?? 'new']) }}">
            ...
            <div class="px-4 pt-2">
                <livewire:pages::transaksi.ri.display-pasien-ri.display-pasien-ri :riHdrNo="$riHdrNo"
                    wire:key="kriteria-robson-display-pasien-{{ $riHdrNo }}" />
            </div>
