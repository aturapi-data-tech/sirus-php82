    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Buka kunci Laporan Anestesi (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut, entri kembali draft', 'MR');
            });
                    ...
                    <div class="flex items-center flex-1 gap-3 min-w-0">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div class="flex items-baseline gap-2 min-w-0">
                                ...
                                <p class="flex-1 min-w-0 truncate text-xs text-muted dark:text-gray-400">PAB 6 / RM 53 — ahli anestesiologi. Tiap entri = 1 laporan; kunci lewat TTD.</p>
                            </div>
                        </div>
                                                ...
                                                <tr @click="open = !open"
                                                    ...
                                                    <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                        <div class="flex items-center justify-end gap-2">
                                                            ...
                                                            @endif
                                                            <x-cetak-button wire:click="cetak('{{ $rowKey }}')" title="Cetak laporan ini" />
                                                            </div>
                                                            @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                                ...
                                                                @if ($isFinal)
                                                                    @can('dokumen.bukaKunci')
                                                                        <x-confirm-button variant="warning-soft" action="bukaKunci('{{ $rowKey }}')" title="Buka Kunci Laporan Anestesi"
                                                                ...
                                                                @endif
                                                                @can('dokumen.hapus')
                                                                <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus laporan ini?" />
                                                                @endcan
                                                                </div>
                                                            @endif
                                                        </div>
