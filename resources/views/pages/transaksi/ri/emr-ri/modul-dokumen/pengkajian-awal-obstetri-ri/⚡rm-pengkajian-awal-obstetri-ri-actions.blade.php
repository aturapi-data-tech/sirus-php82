        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    <h3 class="truncate shrink-0 text-base font-semibold text-ink dark:text-gray-200">Pengkajian Awal Obstetri</h3>
                    @if ($paCount > 0)
                    ...
                    @endif
                    <x-deskripsi-ringkas class="hidden sm:flex text-sm">Pengkajian awal pasien obstetri (RM 44/44.a) — identitas & sosial, status obstetri (G-P-A), riwayat persalinan, TTV, pemeriksaan dalam, skrining (PP 1.2), diagnosa & rencana. Diisi Bidan/Dokter.</x-deskripsi-ringkas>
                </div>
            </div>
                ...
                <div class="flex items-center gap-3 min-w-0 flex-1 min-w-0">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="flex items-baseline gap-2 min-w-0">
                            <h2 class="text-sm truncate shrink-0 font-semibold text-ink dark:text-gray-100">Pengkajian Awal Obstetri</h2>
                            <p class="flex-1 min-w-0 truncate text-xs text-muted dark:text-gray-400">RM 44 / 44.a — kebidanan (VK). Diisi Bidan / Dokter.</p>
                        </div>
                    </div>
                                ...
                                <table class="min-w-full text-sm">
                                    <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                            ...
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Diagnosa</th>
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                    <div class="flex items-center justify-end gap-2">
                                                        ...
                                                        @endif
                                                        <x-cetak-button wire:click="cetak('{{ $rowKey }}')" title="Cetak PDF" />
                                                        </div>
                                                        @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                            ...
                                                            @endif
                                                            @can('dokumen.hapus')
                                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus entri pengkajian ini?" />
                                                            @endcan
                                                            </div>
                                                        @endif
                                                    </div>
