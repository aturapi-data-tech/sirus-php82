...
// Draft + Lanjutkan Pengisian + TTD-Kunci + Lihat(read-only) + tabel expandable, disimpan ke datadaftarri_json.
...
<div>
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    <h3 class="truncate shrink-0 text-base font-semibold text-ink dark:text-gray-200">Instruksi Pasca Bedah</h3>
                    @if ($ipbCount > 0)
                        <x-badge class="shrink-0 whitespace-nowrap" variant="success">{{ $ipbCount }} instruksi</x-badge>
                    @else
            ...
            <div class="mt-3 overflow-x-auto rounded-2xl border border-hairline dark:border-gray-700">
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        @forelse (collect($instruksiList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $e)
    ...
    <x-modal name="rm-instruksi-pasca-bedah-ri-{{ $riHdrNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            ...
            <x-modul-dokumen.header judul="Instruksi Pasca Bedah"
                ikon="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"
                jalur="RI" :jumlah="count($instruksiList)" :readOnly="$isFormLocked">
                                ...
                                <table class="min-w-full text-sm">
                                    <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                            ...
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Ringkasan</th>
                                    ...
                                    @forelse (collect($instruksiList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                                        @php
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                    <div class="flex items-center justify-end gap-2">
                                                        ...
                                                        @endif
                                                        <x-cetak-button wire:click="cetak('{{ $rowKey }}')" title="Cetak instruksi ini" />
                                                        </div>
                                                        @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                            ...
                                                            @if ($isFinal)
                                                                @can('dokumen.bukaKunci')
                                                                    <x-confirm-button variant="warning-soft" action="bukaKunci('{{ $rowKey }}')" title="Buka Kunci Instruksi Pasca Bedah"
                                                            ...
                                                            @endif
                                                            @can('dokumen.hapus')
                                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus instruksi ini?" />
                                                            @endcan
                                                            </div>
                                                        @endif
                                                    </div>
