...
<div>
    ...
    <x-modul-dokumen.kartu judul="Surveilans Pneumonia Ventilator (VAP)"
        :jumlah="$jumlahEntri"
        satuan="entri"
        :nonaktif="$disabled || !$riHdrNo">
        <x-slot:deskripsi>Pemantauan pneumonia terkait pemakaian ventilator — lama pemasangan, demam, sekresi dahak purulen, rasio FiO2/PO2, gambaran foto toraks, serta kultur aspirat / biopsi. Diisi IPCLN / Perawat ruangan (ICU).</x-slot:deskripsi>
                        ...
                        <div class="overflow-x-auto rounded-2xl">
                            <table class="min-w-full text-sm">
                                    ...
                                    <tbody wire:key="surv-entri-{{ $rowKey ?: $loop->index }}" x-data="{ open: false }"
                                        ...
                                        <tr @click="open = !open"
                                            ...
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                ...
                                                <span class="text-muted-soft">·</span> Pasang: {{ $entri['tglPasang'] ?: '-' }}
                                            </td>
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                {{ $entri['dokterMerawat'] ?: '-' }}
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$rowFinal" :terkunci="$isFormLocked"
                                                    ...
                                                    judulBukaKunci="Buka Kunci Surveilans VAP"
                                        ...
                                        <tr x-show="open" x-cloak>
                                            <td colspan="7" class="px-4 py-4 bg-surface-soft/60 dark:bg-gray-950/30">
                                                <dl class="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                                                    ...
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Ventilator</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['ventilator'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Pemakaian Ventilator</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ collect([$entri['tglPasang'] ?? null, $entri['tglLepas'] ?? null])->filter(fn($bagian) => filled($bagian))->implode(' s/d ') ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Demam</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['demam'] ?: '-' }}@if (filled($entri['demamHariKe'] ?? null)) <span class="text-muted">(hari ke-{{ $entri['demamHariKe'] }})</span>@endif
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Sekresi Purulen</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['sekresiPurulen'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">FiO2 Hari Ke</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            &ge; 240: {{ $entri['fio2Ge240HariKe'] ?: '-' }} <span class="text-muted-soft">·</span> &lt; 240: {{ $entri['fio2Lt240HariKe'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Foto Toraks</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ collect((array) ($entri['fotoToraks'] ?? []))->filter()->keys()->map(fn($kunci) => \App\Support\Options\SurveilansHaisOptions::FOTO_TORAKS[$kunci] ?? $kunci)->implode(', ') ?: '-' }}@if (filled($entri['fotoToraksKeterangan'] ?? null)) <span class="text-muted">({{ $entri['fotoToraksKeterangan'] }})</span>@endif
                                                        </dd>
                                                    </div>
                                ...
                                @empty
                                    <tbody>
                                        <tr>
                                            <td colspan="7" class="px-4 py-8 text-center text-muted-soft">Belum ada entri surveilans pneumonia ventilator.</td>
                                        </tr>
                                    </tbody>
                                @endforelse
                            </table>
