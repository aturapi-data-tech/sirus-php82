...
<div>
    ...
    <x-modul-dokumen.kartu judul="Surveilans Infeksi Aliran Darah Primer & Plebitis"
        :jumlah="$jumlahEntri"
        satuan="entri"
        :nonaktif="$disabled || !$riHdrNo">
        <x-slot:deskripsi>Pemantauan infeksi aliran darah primer (IADP) &amp; plebitis pada pemasangan kateter perifer / vena sentral / umbilikal — lokasi &amp; lama pemasangan, tanda infeksi per kelompok usia, tujuan pemasangan, serta hasil kultur darah &amp; pus. Diisi IPCLN / Perawat ruangan.</x-slot:deskripsi>
    ...
    <x-modal name="rm-surveilans-plebitis-ri-{{ $riHdrNo }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Surveilans Infeksi Aliran Darah Primer & Plebitis"
                ikon="M12 3l5.5 6.5a5.5 5.5 0 11-11 0L12 3z"
                jalur="RI" :jumlah="count($entriList)" :readOnly="$isFormLocked">
                Formulir Surveilans HAIs — diisi IPCLN / Perawat ruangan.
                                                    ...
                                                    <tr wire:key="pasang-{{ $indeks }}" class="align-top bg-canvas dark:bg-gray-900">
                                                        ...
                                                        @unless ($formReadOnly)
                                                            <td class="px-3 py-2 text-center">
                                                                <x-hapus-button wire:click.prevent="hapusPemasangan({{ $indeks }})" confirm="Hapus baris pemasangan ini dari daftar?" title="Hapus dari daftar" />
                                                            </td>
                                                        @endunless
                                                    </tr>
                                                @endforeach
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                    <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                        ...
                                        <th class="whitespace-nowrap px-4 py-3 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700 bg-surface-card dark:bg-gray-800">Akses Vaskular</th>
                                    ...
                                    <tbody wire:key="surv-entri-{{ $rowKey ?: $loop->index }}" x-data="{ open: false }"
                                        ...
                                        <tr @click="open = !open"
                                            ...
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                {{ \App\Support\Options\SurveilansHaisOptions::KELOMPOK_USIA[$entri['kelompokUsia'] ?? ''] ?? '-' }}
                                                <span class="text-muted-soft">·</span> Lokasi: {{ collect($entri['pemasangan'] ?? [])->pluck('lokasi')->filter()->implode(', ') ?: '-' }}
                                            </td>
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                {{ $entri['dokterMerawat'] ?: '-' }}
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$rowFinal" :terkunci="$isFormLocked"
                                                    ...
                                                    judulBukaKunci="Buka Kunci Surveilans IADP &amp; Plebitis"
                                                    pesanBukaKunci="Buka kunci entri surveilans ini? TTD petugas akan dicabut."
                                                    konfirmasiHapus="Yakin hapus entri surveilans ini?" />
                                            </td>
                                        </tr>
                                        ...
                                        <tr x-show="open" x-cloak>
                                            <td colspan="7" class="px-4 py-4 bg-surface-soft/60 dark:bg-gray-950/30">
                                                <dl class="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                                                    ...
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Kateter Perifer / Sentral / Umbilikal</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['kateterPerifer'] ?: '-' }} / {{ $entri['kateterVCentral'] ?: '-' }} / {{ $entri['kateterUmbilikal'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Kelompok Usia</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ \App\Support\Options\SurveilansHaisOptions::KELOMPOK_USIA[$entri['kelompokUsia'] ?? ''] ?? '-' }}
                                                        </dd>
                                                    </div>
                                                    <div class="md:col-span-2">
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Tujuan Pemasangan</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ collect((array) ($entri['tujuanPemasangan'] ?? []))->filter()->keys()->map(fn($kunci) => \App\Support\Options\SurveilansHaisOptions::TUJUAN_PEMASANGAN[$kunci] ?? $kunci)->implode(', ') ?: '-' }}@if (filled($entri['tujuanKeterangan'] ?? null)) <span class="text-muted">({{ $entri['tujuanKeterangan'] }})</span>@endif
                                                        </dd>
                                                    </div>
                                                    <div class="md:col-span-2">
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Riwayat Pemasangan</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            @forelse ((array) ($entri['pemasangan'] ?? []) as $barisPasang)
                                                                @php
                                                                    $tandaPasang = collect((array) ($barisPasang['tanda'] ?? []))->filter()->keys()
                                                                        ->map(fn($kunci) => (\App\Support\Options\SurveilansHaisOptions::TANDA_IADP_BALITA + \App\Support\Options\SurveilansHaisOptions::TANDA_IADP_DEWASA)[$kunci] ?? $kunci)->implode(', ');
                                                                    $rentangPasang = collect([$barisPasang['tglMulai'] ?? null, $barisPasang['tglSelesai'] ?? null])->filter(fn($bagian) => filled($bagian))->implode(' s/d ');
                                                                @endphp
                                                                <div class="flex flex-wrap items-center gap-2">
                                                                    <span class="font-medium">{{ \App\Support\Options\SurveilansHaisOptions::JENIS_AKSES[$barisPasang['jenisAkses'] ?? ''] ?? (($barisPasang['jenisAkses'] ?? '') ?: '-') }}@if (filled($barisPasang['lokasi'] ?? null)) <span class="text-muted">· {{ $barisPasang['lokasi'] }}</span>@endif</span>
                                                                    @if ($rentangPasang)
                                                                        <span class="text-muted dark:text-gray-400">{{ $rentangPasang }}</span>
                                                                    @endif
                                                                    @if (filled($barisPasang['hariKe'] ?? null))
                                                                        <span class="text-muted dark:text-gray-400">hari ke-{{ $barisPasang['hariKe'] }}</span>
                                                                    @endif
                                                                    @if ($tandaPasang)
                                                                        <x-badge variant="danger">{{ $tandaPasang }}</x-badge>
                                                                    @endif
                                                                </div>
                                                            @empty
                                                                -
                                ...
                                @empty
                                    <tbody>
                                        <tr>
                                            <td colspan="7" class="px-4 py-8 text-center text-muted-soft">Belum ada entri surveilans IADP &amp; plebitis.</td>
                                        </tr>
                                    </tbody>
                                @endforelse
                            </table>
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                ...
                :mengedit="$editingKey">
                <x-slot:tombol>
                    @if ($editingKey && !$isFormLocked && !$viewOnly)
