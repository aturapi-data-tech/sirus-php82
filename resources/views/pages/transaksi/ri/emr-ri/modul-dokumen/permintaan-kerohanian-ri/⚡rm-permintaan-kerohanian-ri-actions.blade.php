...
<div>
    ...
    <x-modul-dokumen.kartu judul="Permintaan Pelayanan Kerohaniawan"
        :jumlah="$pkCount"
        satuan="catatan"
        :nonaktif="$disabled || !$riHdrNo">
        <x-slot:deskripsi>Formulir permintaan pendampingan pelayanan kerohanian sesuai agama/kepercayaan pasien selama Rawat Inap. Dapat lebih dari satu catatan.</x-slot:deskripsi>
        <div class="overflow-x-auto rounded-2xl border border-hairline dark:border-gray-700">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    @forelse (collect($permintaanList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $pk)
                        <tr class="border-b border-hairline dark:border-gray-700">
                            ...
                            <td class="px-3 py-2 text-muted dark:text-gray-400">
                                <x-modul-dokumen.status-ttd :nama="$pk['petugas'] ?? ''" gaya="polos" />
                            </td>
                            <td class="px-3 py-2 text-muted dark:text-gray-400">{{ $pk['agama'] ?? '-' }}</td>
                            <td class="px-3 py-2 text-center">
                                <x-modul-dokumen.status-entri :final="$this->entryIsFinal($pk)" />
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <x-modal name="rm-permintaan-kerohanian-ri-{{ $riHdrNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Permintaan Pelayanan Kerohaniawan"
                ...
                jalur="RI" :jumlah="count($permintaanList)" :readOnly="$isFormLocked">
                                ...
                                <table class="min-w-full text-base">
                                    <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                            ...
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Pemohon</th>
                                            ...
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Agama</th>
                                    ...
                                    @forelse (collect($permintaanList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                                        @php
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 align-middle text-center whitespace-nowrap" @click.stop>
                                                    <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                        ...
                                                        judulBukaKunci="Buka Kunci Permintaan Pelayanan Kerohanian"
                                                        konfirmasiHapus="Yakin hapus permintaan ini?" />
                                                </td>
                                            </tr>
