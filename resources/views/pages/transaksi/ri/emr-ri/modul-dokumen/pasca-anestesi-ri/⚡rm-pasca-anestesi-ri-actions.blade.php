    ...
    <x-modal name="rm-pasca-anestesi-ri-{{ $riHdrNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Monitoring Pasca Anestesi"
                ...
                jalur="RI" :jumlah="count($pascaList)" :readOnly="$isFormLocked">
                                    ...
                                    <tbody class="divide-y divide-hairline dark:divide-gray-700">
                                        @forelse ($obsRows as $i => $row)
                                            <tr wire:key="obs-{{ $i }}-{{ $row['waktu'] ?? '' }}">
                                                ...
                                                @unless ($viewOnly)
                                                    <td class="px-3 py-2 text-center">
                                                        <x-hapus-button wire:click.prevent="removeObservasi({{ $i }})" />
                                                    </td>
                                                @endunless
                                            </tr>
                                        @empty
                                ...
                                <table class="min-w-full text-sm">
                                    <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                            ...
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Jam Masuk</th>
                                            <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Aldrete</th>
                                    ...
                                    @forelse (collect($pascaList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                                        @php
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                    <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                        ...
                                                        judulBukaKunci="Buka Kunci Monitoring Pasca Anestesi"
                                                        pesanBukaKunci="TTD petugas RR akan dicabut & entri kembali menjadi Draft — proses TTD diulang dari awal. Lanjutkan?"
                                                        konfirmasiHapus="Yakin hapus monitoring ini?" />
                                                </td>
                                            </tr>
