...
<div>
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    <h3 class="truncate shrink-0 text-base font-semibold text-ink dark:text-gray-200">Surveilans Infeksi Saluran Kemih</h3>
                    @if ($jumlahEntri > 0)
                    ...
                    @endif
                    <x-deskripsi-ringkas class="hidden sm:flex text-sm">Pemantauan ISK terkait pemasangan kateter urin (SPP / douer / intermiten / kondom) — lama pemasangan, tanda infeksi per kelompok usia, pemeriksaan leukosit urin &amp; biakan urin. Diisi IPCLN / Perawat ruangan.</x-deskripsi-ringkas>
                </div>
            </div>
    ...
    <x-modal name="rm-surveilans-isk-ri-{{ $riHdrNo }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Surveilans Infeksi Saluran Kemih"
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                    <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                        ...
                                        <th class="whitespace-nowrap px-4 py-3 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700 bg-surface-card dark:bg-gray-800">Kateter</th>
                                    ...
                                    <tbody wire:key="surv-entri-{{ $rowKey ?: $loop->index }}" x-data="{ open: false }"
                                        ...
                                        <tr @click="open = !open"
                                            ...
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                ...
                                                <span class="text-muted-soft">·</span> Kateter: {{ \App\Support\Options\SurveilansHaisOptions::JENIS_KATETER_ISK[$entri['jenisKateter'] ?? ''] ?? '-' }}
                                            </td>
                                            <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                                {{ $entri['dokterMerawat'] ?: '-' }}
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$rowFinal" :terkunci="$isFormLocked"
                                                    ...
                                                    judulBukaKunci="Buka Kunci Surveilans ISK"
                                        ...
                                        <tr x-show="open" x-cloak>
                                            <td colspan="7" class="px-4 py-4 bg-surface-soft/60 dark:bg-gray-950/30">
                                                <dl class="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                                                    ...
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Jenis Kateter</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ \App\Support\Options\SurveilansHaisOptions::JENIS_KATETER_ISK[$entri['jenisKateter'] ?? ''] ?? '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">Tanggal Pasang Kateter</dt>
                                                        <dd class="mt-0.5 text-ink dark:text-gray-200">
                                                            {{ $entri['tglPasangKateter'] ?: '-' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                            ...
                                                            @forelse ((array) ($entri['pemasangan'] ?? []) as $barisPasang)
                                                                @php
                                                                    $tandaPasang = collect((array) ($barisPasang['tanda'] ?? []))->filter()->keys()
                                                                        ->map(fn($kunci) => (\App\Support\Options\SurveilansHaisOptions::TANDA_ISK_BALITA + \App\Support\Options\SurveilansHaisOptions::TANDA_ISK_DEWASA)[$kunci] ?? $kunci)->implode(', ');
                                                                ...
                                                                <div class="flex flex-wrap items-center gap-2">
                                                                    <span class="font-medium">Pemasangan ke-{{ $barisPasang['pasangKe'] ?? '-' }}</span>
                                                                    @if ($rentangPasang)
                                ...
                                @empty
                                    <tbody>
                                        <tr>
                                            <td colspan="7" class="px-4 py-8 text-center text-muted-soft">Belum ada entri surveilans ISK.</td>
                                        </tr>
                                    </tbody>
                                @endforelse
                            </table>
