...
new class extends Component {
    ...
    /* ===============================
     ...
     =============================== */
    #[On('lov.selected.surveilans-isk-room')]
    ...
    #[On('lov.selected.surveilans-isk-dokter')]
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="w-full space-y-4">
                    ...
                    <fieldset @disabled($formRO) class="space-y-4">
                                ...
                                @unless ($formRO)
                                    <div class="p-3 mt-3 border border-dashed rounded-lg border-gray-300 dark:border-gray-600 bg-canvas dark:bg-gray-800/50">
                                        ...
                                        <div class="grid items-end grid-cols-1 gap-3 lg:grid-cols-5">
                                            <div>
                                                <livewire:lov.room.lov-room target="surveilans-isk-room" label="Ruang"
                                                    placeholder="Ketik nama ruangan / bed..." :initialRoomId="$barisRawat['roomId'] ?: null"
                                                    wire:key="lov-room-surveilans-isk-{{ $riHdrNo }}-{{ $renderVersions['modal-surveilans-isk-ri'] ?? 0 }}" />
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                <livewire:lov.dokter.lov-dokter target="surveilans-isk-dokter" label="Dokter yang Merawat"
                                                    placeholder="Ketik nama/kode dokter..." :initialDrId="$barisRawat['drId'] ?: null"
                                                    wire:key="lov-dokter-surveilans-isk-{{ $riHdrNo }}-{{ $renderVersions['modal-surveilans-isk-ri'] ?? 0 }}" />
                                            </div>
                                            <div>
                        ...
                        {{-- 4. PEMERIKSAAN URIN --}}
                        <x-border-form title="4. Pemeriksaan & Biakan Urin" :collapsible="true" :open="false">
                            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                <div class="space-y-3">
                                    <div class="flex items-center gap-3">
                                        <x-input-label value="Pemeriksaan Urin Dilakukan" class="shrink-0" />
                                        <x-toggle wire:model="newForm.pemeriksaanUrin" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['pemeriksaanUrin'] ?? null) ? $newForm['pemeriksaanUrin'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                    <x-surveilans.kultur-list list="pemeriksaanUrinHasil" title="Hasil Pemeriksaan Urin"
                                        :rows="$newForm['pemeriksaanUrinHasil'] ?? []" :baris="$barisKultur['pemeriksaanUrinHasil'] ?? []"
                                        :formRO="$formRO" hasilLabel="Leukosit Urin" hasilPlaceholder="Leukosit urin"
                                        kosongTeks="Belum ada hasil pemeriksaan urin." />
                                </div>
                                <div class="space-y-3">
                                    <div class="flex items-center gap-3">
                                        <x-input-label value="Biakan Urin Dilakukan" class="shrink-0" />
                                        <x-toggle wire:model="newForm.biakanUrin" trueValue="Ya" falseValue="Tidak"
                                            :label="filled($newForm['biakanUrin'] ?? null) ? $newForm['biakanUrin'] : 'Belum diisi'" :disabled="$formRO" />
                                    </div>
                                    <x-surveilans.kultur-list list="biakanUrinHasil" title="Hasil Biakan Urin"
                                        :rows="$newForm['biakanUrinHasil'] ?? []" :baris="$barisKultur['biakanUrinHasil'] ?? []"
