<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/penolakan-resusitasi-ri/rm-penolakan-resusitasi-ri-actions.blade.php
//
// Surat Pernyataan Penolakan Tindakan Resusitasi (DNR / Do Not Resuscitate) — modul dokumen RI.
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-penolakan-resusitasi-ri'];
    ...
    public function mount(?string $riHdrNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-penolakan-resusitasi-ri']);
    ...
    public function openModal(): void
    {
        ...
        $this->dataDaftarRi = $data;
        $this->regNo = $data['regNo'] ?? null;
        if (!isset($this->dataDaftarRi['penolakanResusitasiRI']) || !is_array($this->dataDaftarRi['penolakanResusitasiRI'])) {
            $this->dataDaftarRi['penolakanResusitasiRI'] = [];
        }
        $this->penolakanList = $this->dataDaftarRi['penolakanResusitasiRI'];
        ...
        $this->dispatch('open-modal', name: "rm-penolakan-resusitasi-ri-{$this->riHdrNo}");
    }
    ...
    {
        $this->dispatch('close-modal', name: "rm-penolakan-resusitasi-ri-{$this->riHdrNo}");
    }
    ...
    private function persistEntry(string $key, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $key, $logVerb) {
            ...
            if (!isset($data['penolakanResusitasiRI']) || !is_array($data['penolakanResusitasiRI'])) {
                $data['penolakanResusitasiRI'] = [];
            }

            $list = $data['penolakanResusitasiRI'];
    ...
    public function bukaKunci(string $key): void
    {
        ...
        try {
            DB::transaction(function () use ($key) {
                ...
                $list = $fresh['penolakanResusitasiRI'] ?? [];
    ...
    public function cetak(string $signatureDate)
    {
        ...
        try {
            ...
            $pdf = Pdf::loadView('pages.components.modul-dokumen.ri.penolakan-resusitasi-ri.cetak-penolakan-resusitasi-ri-print', ['data' => $data])->setPaper('A4');

            $this->dispatch('toast', type: 'success', message: 'Berhasil mencetak surat pernyataan penolakan resusitasi (DNR).');
            return response()->streamDownload(fn() => print $pdf->output(), 'penolakan-resusitasi-ri-' . ($pasien['regNo'] ?? $this->riHdrNo) . '.pdf');
        } catch (\Throwable $e) {
    ...
    public function hapus(string $signatureDate): void
    {
        ...
        try {
            DB::transaction(function () use ($signatureDate) {
                ...
                $data = $this->findDataRI($this->riHdrNo);
                if (empty($data) || !isset($data['penolakanResusitasiRI'])) {
                    throw new \RuntimeException('Data formulir tidak ditemukan.');
                }

                $data['penolakanResusitasiRI'] = collect($data['penolakanResusitasiRI'])
        ...
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            ...
            <div class="px-4 pt-2">
                <livewire:pages::transaksi.ri.display-pasien-ri.display-pasien-ri :riHdrNo="$riHdrNo"
                    wire:key="dnr-ri-display-pasien-{{ $riHdrNo ?? 'init' }}" />
            </div>
