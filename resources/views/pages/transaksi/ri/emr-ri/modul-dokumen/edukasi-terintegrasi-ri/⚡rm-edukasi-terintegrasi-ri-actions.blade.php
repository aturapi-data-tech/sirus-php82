...
new class extends Component {
    ...
    public function setSasaranSignature(string $dataUrl): void
    {
        ...
        $this->sasaranEdukasiSignature = TtdPasien::simpan($dataUrl, $this->regNo);   // gambar ke RSTXN_TTDS; di sini cukup referensi "TTD:<no>"
...
<div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-surface-card dark:bg-gray-800">
                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                            ...
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Pasien / Keluarga</th>
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Petugas (TTD)</th>
    ...
    <x-modal name="rm-edukasi-terintegrasi-ri-{{ $riHdrNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full">
            <x-modul-dokumen.header judul="Edukasi Terintegrasi"
                ...
                :jumlah="$jumlahEdukasiTerintegrasi" :readOnly="$isFormLocked">
                Edukasi pasien &amp; keluarga oleh seluruh profesi — kebutuhan, metode, hasil, dan tindak lanjutnya.
                <x-slot:badge>
                    @if ($this->diForm())
    ...
    @if ($this->diForm() && $viewOnly)
        <x-modul-dokumen.banner jenis="lihat" class="mb-2">
    ...
    @elseif ($this->diForm() && $editingKey)
        <x-modul-dokumen.banner jenis="lanjut" class="mb-2" />
    @endif
    ...
    <x-modul-dokumen.tabel-daftar :kolom="['', 'Tanggal', 'Pasien / Keluarga', 'Petugas (TTD)', 'Status' => 'text-center', 'Aksi' => 'text-center w-64']">
                    ...
                    <tbody wire:key="edu-terint-{{ $edukasiId ?: $loop->index }}"
                        ...
                        <tr @click="open = !open"
                            ...
                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                <x-modul-dokumen.aksi-entri kunci="{{ $edukasiId }}" :final="$isFinal" :terkunci="$isFormLocked"
                                    :bisaDiubah="filled($edukasiId)"
                                    hapus="removeEdukasiTerintegrasiById"
                                    judulLihat="Lihat detail (read-only) di form atas"
                                    judulBukaKunci="Buka Kunci Edukasi Terintegrasi"
                                    pesanBukaKunci="TTD petugas akan dicabut & entri kembali menjadi draft untuk dikoreksi. TTD pasien/keluarga tetap. Lanjutkan?"
                                    konfirmasiHapus="Hapus data edukasi terintegrasi ini?" />
                            </td>
                        </tr>
                ...
                @empty
                    <x-modul-dokumen.baris-kosong :kolom="6" pesan="Belum ada data edukasi terintegrasi." />
                @endforelse
    ...
    @endunless
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                ...
                :mengedit="$editingKey"
                selesaiLihat="">
                Simpan draft dulu; setelah TTD pasien/keluarga, <strong>kunci</strong> lewat tombol <strong>TTD Petugas &amp; Kunci</strong>.
