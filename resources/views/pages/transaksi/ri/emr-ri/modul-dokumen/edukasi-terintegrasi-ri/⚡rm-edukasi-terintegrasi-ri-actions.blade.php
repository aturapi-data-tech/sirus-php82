                            ...
                            <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if ($isFinal && $edukasiId)
                                        <x-lihat-button wire:click="viewEntry('{{ $edukasiId }}')" title="Lihat detail (read-only) di form atas" />
                                    @endif
                                    @if ($edukasiId)
                                        <x-cetak-button wire:click="cetak('{{ $edukasiId }}')" title="Cetak" />
                                    @endif
                                    </div>
                                    ...
                                    @if (!$isFormLocked && $edukasiId && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                        ...
                                        @endif
                                        @can('dokumen.hapus')
                                        <x-hapus-button wire:click.prevent="removeEdukasiTerintegrasiById('{{ $edukasiId }}')" confirm="Hapus data edukasi terintegrasi ini?" />
                                        @endcan
                                        </div>
                                    @endif
                                </div>
