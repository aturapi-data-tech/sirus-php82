        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    <h3 class="truncate shrink-0 text-base font-semibold text-ink dark:text-gray-200">Observasi Nifas</h3>
                    @if ($onCount > 0)
                        <x-badge class="shrink-0 whitespace-nowrap" variant="success">{{ $onCount }} lembar</x-badge>
                    @else
                        <x-badge class="shrink-0 whitespace-nowrap" variant="warning">Belum ada</x-badge>
                    @endif
                    <x-deskripsi-ringkas class="hidden sm:flex text-sm">Lembar pemantauan masa nifas (post-partum) per titik-waktu — TD, nadi, RR, suhu, Maternal Early Warning Score, TFU, kontraksi uterus, lochia, perdarahan, luka jalan lahir, laktasi, ASI eksklusif &amp; rawat gabung. Satu entri = satu lembar berisi banyak titik-waktu.</x-deskripsi-ringkas>
                </div>
            </div>
                ...
                <div class="flex items-center gap-3 min-w-0 flex-1 min-w-0">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="flex items-baseline gap-2 min-w-0">
                            <h2 class="text-sm truncate shrink-0 font-semibold text-ink dark:text-gray-100">Observasi Nifas</h2>
                            <x-deskripsi-ringkas class="text-xs">Lembar pemantauan masa nifas (VK) — satu lembar berisi banyak titik-waktu. Diisi Bidan / Perawat.</x-deskripsi-ringkas>
                        </div>
                    </div>
                        ...
                        <x-border-form title="Observasi Nifas (Titik-Waktu)">
                            <div class="space-y-2">
                                <div class="overflow-x-auto bg-canvas border rounded-2xl border-hairline dark:border-gray-700">
                                    <table class="ds-table ds-table-entri min-w-[1840px]">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($newForm['rows'] ?? [] as $nomor => $baris)
                                                <tr wire:key="observasi-nifas-baris-{{ $nomor }}">
                                                    ...
                                                    <td class="ds-c">
                                                        @if (!$formReadOnly)
                                                            <x-hapus-button :action="'hapusBaris(' . $nomor . ')'" title="Hapus Baris" :message="'Yakin hapus baris titik-waktu ' . (($baris['tglJam'] ?? '') ?: 'ini') . ' dari lembar?'" />
                                                        @else
                                                            <span class="text-muted-soft">—</span>
                                                        @endif
                                                    </td>
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                    <div class="flex items-center justify-end gap-2">
                                                        ...
                                                        @endif
                                                        <x-cetak-button wire:click="cetakLembar('{{ $rowKey }}')" title="Cetak lembar observasi ini" />
                                                        </div>
                                                        @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                            ...
                                                            @endif
                                                            @can('dokumen.hapus')
                                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus baris observasi ini?" />
                                                            @endcan
                                                            </div>
                                                        @endif
                                                    </div>
                                                        ...
                                                        <table class="ds-table">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="whitespace-nowrap">TFU</th>
                                                                    <th class="whitespace-nowrap">Kontraksi</th>
                                                                    <th class="whitespace-nowrap">Lochia</th>
                                                                    <th class="whitespace-nowrap">Drh (cc)</th>
                                                                    <th class="whitespace-nowrap">Luka</th>
                                                                    <th class="whitespace-nowrap">BAK</th>
                                                                    <th class="whitespace-nowrap">BAB</th>
                                                                    <th class="whitespace-nowrap">Laktasi</th>
                                                                    <th class="whitespace-nowrap">ASI</th>
                                                                    <th class="whitespace-nowrap">Rawat Gabung</th>
                                                                    <th class="whitespace-nowrap">Mobilisasi</th>
                                                                    <th class="whitespace-nowrap">Keluhan</th>
                                                                    <th class="whitespace-nowrap">Asuhan / Tindakan</th>
