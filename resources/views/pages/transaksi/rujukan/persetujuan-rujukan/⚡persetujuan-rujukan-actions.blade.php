...
new class extends Component {
    ...
    {
        return ($this->permintaan['keputusan'] ?? '') === ''
            && ($this->permintaan['statusTask'] ?? '') !== 'cancelled';
    }
    ...
    private function jawab(string $keputusan): void
    {
        ...
        $this->dispatch(
            'toast',
            type: 'success',
            message: $keputusan === 'accepted'
                ? 'Permintaan rujukan DISETUJUI dan sudah dikirim ke SATUSEHAT.'
                : 'Permintaan rujukan DITOLAK dan sudah dikirim ke SATUSEHAT.',
        );
...
<div>
    <x-modal name="persetujuan-rujukan-actions" size="3xl" focusable>
        @php
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="p-4 border bg-surface-soft border-hairline rounded-xl dark:bg-gray-800 dark:border-gray-700">
                    <div class="text-xs font-semibold tracking-wide uppercase text-muted dark:text-gray-400">Pasien</div>
                    <div class="mt-1 font-semibold text-ink dark:text-gray-100">
                        {{ ($permintaan['pasienNama'] ?? '') !== '' ? $permintaan['pasienNama'] : '(nama tidak dikirim perujuk)' }}
                    </div>
                ...
                <div class="p-4 border bg-surface-soft border-hairline rounded-xl dark:bg-gray-800 dark:border-gray-700">
                    <div class="text-xs font-semibold tracking-wide uppercase text-muted dark:text-gray-400">RS Perujuk
                    </div>
                    <div class="mt-1 font-semibold text-ink dark:text-gray-100">
            ...
            <div class="p-4 border bg-canvas border-hairline rounded-xl dark:bg-gray-900 dark:border-gray-700">
                ...
                <p class="mt-1 text-sm whitespace-pre-line text-body dark:text-gray-200">
                    {{ ($permintaan['deskripsi'] ?? '') !== '' ? $permintaan['deskripsi'] : 'Perujuk tidak mengisi keterangan klinis.' }}
                </p>
            ...
            <details class="p-4 border bg-surface-soft border-hairline rounded-xl dark:bg-gray-800 dark:border-gray-700">
                ...
                <dl class="grid grid-cols-1 gap-2 mt-3 text-sm sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <dd class="break-all text-body dark:text-gray-200">{{ $permintaan['rencanaId'] ?? '-' }}</dd>
                    </div>
                    <div>
            ...
            @if (!$this->menunggu())
                <div class="p-3 text-sm border rounded-xl bg-blue-50 border-blue-200 text-info-deep dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-200">
                    @if ($statusTask === 'cancelled')
            ...
            {{-- AKSI --}}
            <div class="flex flex-wrap justify-end gap-2 pt-2 border-t border-hairline dark:border-gray-700">
                <x-secondary-button type="button" x-on:click="$dispatch('close-modal', { name: 'persetujuan-rujukan-actions' })">
                    Tutup
                </x-secondary-button>
