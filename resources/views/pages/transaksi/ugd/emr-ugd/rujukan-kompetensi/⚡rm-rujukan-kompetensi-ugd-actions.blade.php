...
new class extends Component {
    ...
    public function mount(): void
    {
        ...
        $tersimpan = $dataDaftarUGD['rujukanKompetensi'] ?? [];
    ...
    {
        $diagnosisPertama = collect($this->dataDaftarUGD['diagnosis'] ?? [])->first();
        ...
        $this->formRujukan['kodeSpesialis'] = $this->dataDaftarUGD['kdpolibpjs'] ?? '';
    }
    ...
    private function pasienTidakCocok(): array
    {
        ...
        $kartuPasien = preg_replace('/\D/', '', (string) (DB::table('rsmst_pasiens')
            ...
            ->value('nokartu_bpjs') ?? ''));
        $kartuSep = preg_replace('/\D/', '', (string) ($this->dataDaftarUGD['sep']['reqSep']['request']['t_sep']['noKartu'] ?? ''));
    ...
    {
        $regNo = $this->dataDaftarUGD['regNo'] ?? ($this->dataDaftarUGD['reg_no'] ?? '');
        if ($regNo === '') {
            return '';
        }
    ...
    {
        $drId = $this->dataDaftarUGD['drId'] ?? ($this->dataDaftarUGD['dr_id'] ?? '');
        if ($drId === '') {
            return '';
        }
    ...
    #[On('lov.selected.rujukanRajalDiagnosaUGD')]
     ...
     */
    #[On('lov.selected.rujukanRajalIcd9UGD')]
    ...
    #[On('lov.cleared.rujukanRajalIcd9UGD')]
    ...
    #[On('lov.selected.rujukanRajalSpesialisUGD')]
    ...
    #[On('lov.selected.rujukanRajalPoliUGD')]
     ...
     */
    #[On('lov.selected.rujukanRajalWilayahUGD')]
    ...
    public function hapusRujukan(): void
    {
        ...
        // (gateway menolak: "satuSehatRujukan tidak boleh kosong", 11/09/26).
    ...
    <div class="p-5 bg-canvas border border-hairline shadow-sm rounded-2xl dark:bg-gray-900 dark:border-gray-700">
        <div class="flex flex-col gap-3">
            ...
            <p class="text-base text-muted dark:text-gray-400">
                Rujukan pasien UGD ke poli RS lain lewat BPJS (vclaim-sisrute-rest), yang meneruskannya
            ...
            <div x-data="{ buka: false }"
                ...
                <ul x-show="buka" x-cloak class="px-3 pb-3 ml-4 list-disc text-red-800 dark:text-red-200">
                    @foreach ($prasyaratKurang as $itemKurang)
                        <li>{{ $itemKurang }}</li>
                    @endforeach
                    <li class="list-none -ml-4 pt-1 text-xs">Langkah 1 (Ambil Kriteria) &amp; 2 (Cari Kandidat) tetap bisa dijalankan sambil melengkapi ini.</li>
                </ul>
            </div>
        @endif
            ...
            <div class="max-w-md">
                ...
                <div>
                    <livewire:lov.poli.lov-poli label="Kode Spesialis (poli BPJS)"
                        ...
                        wire:key="lov-poli-spesialis-rajal-ugd-{{ $rjNo }}" />
            ...
            @if (!empty($formRujukan['kriteriaList']))
                    ...
                    @if ($butuhIcd9)
                        <div class="max-w-md space-y-2">
                            <livewire:lov.procedure.lov-procedure label="Cari Tindakan Rujukan (ICD-9-CM)"
                                target="rujukanRajalIcd9UGD" :disabled="$isFormLocked"
                ...
                <div>
                    <livewire:lov.kabupaten.lov-kabupaten label="Jejaring Wilayah Rujukan (Kab/Kota)"
                        target="rujukanRajalWilayahUGD" :initialKabId="$formRujukan['kodeKabupaten'] ?: null"
                        :readonly="$isFormLocked"
                        wire:key="lov-wilayah-rujukan-rajal-ugd-{{ $rjNo }}" />
                    <p class="mt-1 text-xs text-muted-soft">
                        Terpilih:
            ...
            <div class="grid grid-cols-1 gap-3">
                <div>
                    <livewire:lov.poli.lov-poli label="Poli Rujukan (kode BPJS 3 huruf)"
                        ...
                        target="rujukanRajalPoliUGD"
                        :initialPoliId="$this->poliIdPoliRujukan" :disabled="$isFormLocked"
                        wire:key="lov-poli-poli-rujukan-kompetensi-ugd-{{ $rjNo }}" />
                    <p class="mt-1 text-xs text-muted-soft">
