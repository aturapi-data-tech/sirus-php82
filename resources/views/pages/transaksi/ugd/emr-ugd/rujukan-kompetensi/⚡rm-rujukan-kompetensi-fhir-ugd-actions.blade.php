...
new class extends Component {
    ...
    public function openModal(): void
    {
        ...
        $this->dispatch('open-modal', name: 'rujukan-kompetensi-fhir-ugd-' . $this->rjNo);
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: 'rujukan-kompetensi-fhir-ugd-' . $this->rjNo);
    }
...
<div>
    ...
    {{-- ══ MODAL FORMULIR ══ --}}
    <x-modal name="rujukan-kompetensi-fhir-ugd-{{ $rjNo }}" size="full" height="full" focusable>
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                        wire:key="rujukan-kompetensi-fhir-ugd-display-pasien-{{ $rjNo ?? 'init' }}" />
            ...
            <div class="p-3 space-y-3 bg-canvas border border-hairline rounded-lg dark:bg-gray-800 dark:border-gray-700">
                ...
                <div class="max-w-md">
                    <livewire:lov.diagnosa.lov-diagnosa label="Cari Diagnosa Rujukan (ICD-10)" :blockHeader="false"
                        target="rujukanKompetensiDiagnosaUGD"
                        :initialDiagnosaId="$formRujukan['kodeDiagnosa'] ?: null" :disabled="$isFormLocked"
                        wire:key="lov-diagnosa-rujukan-kompetensi-fhir-ugd-{{ $rjNo }}" />
                        ...
                        <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                            @foreach ($formRujukan['kriteriaServer'] ?? [] as $kunciKriteria => $kriteriaServer)
                                <x-radio-button :label="$kriteriaServer['text'] . ($kunciKriteria === 'tindakan' ? ' (ICD-9-CM)' : '')"
                                    :value="$kunciKriteria" name="kriteriaRanapUgd-{{ $rjNo }}"
