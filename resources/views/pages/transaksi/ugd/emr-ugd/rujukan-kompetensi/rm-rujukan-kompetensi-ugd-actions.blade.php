...
new class extends Component {
    ...
    public function cariKandidat(): void
    {
        ...
        if (!collect($this->formRujukan['kriteriaIgd'])->contains(true)) {
        ...
        $kandidat = $this->rujukanTaskPencarianKandidat([
            ...
            'kriteria' => $this->formRujukan['kriteriaIgd'],
        ]);
    ...
    public function kirimTugasRujukan(): void
    {
        ...
        $respon = $this->rujukanBundleApproval([
            ...
            'deskripsi' => trim($this->formRujukan['deskripsi']) !== '' ? trim($this->formRujukan['deskripsi']) : 'Rujukan gawat darurat — ' . $this->formRujukan['kodeDiagnosa'] . ' ' . $this->formRujukan['diagnosaDesc'],
        ...
        $this->simpanDraft('Kirim tugas rujukan IGD → ' . $kandidat['nama']);
    ...
    public function kirimRujukan(): void
    {
        ...
        $respon = $this->rujukanServiceRequest([
            ...
            'jalur' => 'igd',
            'deskripsi' => trim($this->formRujukan['deskripsi']) !== '' ? trim($this->formRujukan['deskripsi']) : 'Rujukan gawat darurat — ' . $this->formRujukan['kodeDiagnosa'],
            'patientUuid' => $this->patientUuid(),
            'encounterId' => $this->encounterUuid(),
            'orgTujuanId' => $kandidat['orgId'],
            'orgTujuanNama' => $kandidat['nama'],
        ...
        $this->simpanDraft('Kirim Rujukan Kompetensi IGD → ' . $kandidat['nama'] . ' (No SS ' . $nomor . ')');
        $this->dispatch('toast', type: 'success', message: 'Rujukan IGD terkirim. No SATUSEHAT ' . $nomor);
    }
...
<div class="w-full p-4 space-y-4 border border-rose-200 rounded-2xl bg-rose-50/50 dark:bg-gray-900 dark:border-rose-700">
    ...
    <div class="flex items-center justify-between">
        <h3 class="font-semibold text-gray-800 dark:text-gray-100">Rujukan Berbasis Kompetensi — IGD (SATUSEHAT FHIR)</h3>
    ...
    @else
        ...
        <div class="p-3 space-y-3 bg-canvas border border-hairline rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <p class="text-sm font-semibold text-gray-700 dark:text-gray-200">1. Diagnosa, Kriteria Gawat Darurat & Kandidat</p>
