{{--
    ...
    Partial: Form entry + riwayat implementasi per diagnosis.
    Berisi checklist tindakan SIKI dan skor evaluasi SLKI (1-5).
    Beda dari versi RI: TANPA SOAP dan tanpa sinkron ke cppt[] — UGD tak punya CPPT.
    Dipakai oleh rm-asuhan-keperawatan-ugd-actions.blade.php via @include.
--}}
            ...
            <div wire:key="impl-{{ $idx }}-{{ $iIdx }}" class="p-3 mb-2 border border-hairline rounded-lg bg-surface-soft/50 dark:bg-gray-800/50 dark:border-gray-700">
                ...
                <div class="flex items-center justify-between mb-2">
                    <div class="flex flex-wrap items-center gap-2 text-sm">
                        <span class="px-1.5 py-0.5 rounded-full text-xs font-bold bg-brand/10 text-brand dark:bg-brand/20 dark:text-brand-lime">Implementasi</span>
                    ...
                    @if (!$isFormLocked)
                        <x-hapus-button wire:click="removeImplementasi({{ $idx }}, {{ $realIdx }})" confirm="Yakin hapus implementasi ini?" />
                    @endif
                </div>
