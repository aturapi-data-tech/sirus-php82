<?php
// resources/views/pages/transaksi/ugd/emr-ugd/asuhan-keperawatan/rm-asuhan-keperawatan-ugd-actions.blade.php
...
new class extends Component {
    ...
    {
        $this->registerAreas(['modal-asuhan-keperawatan-ugd']);
    ...
    #[On('open-rm-asuhan-keperawatan-ugd')]
    ...
    #[On('lov.selected.ugdFormAsuhanKeperawatan')]
    ...
    public function removeAsuhanKeperawatan(int $index): void
    {
        ...
        try {
            $this->withUgdLock(function () use ($index) {
                $fresh = $this->findDataUGD($this->rjNo) ?? [];
    ...
    public function addImplementasi(): void
    {
        ...
        try {
            ...
            $this->withUgdLock(function () use ($idx) {
                $fresh = $this->findDataUGD($this->rjNo) ?? [];
                ...
                // Simpan di dalam askep. TANPA sinkron ke CPPT: UGD memang tak punya
    ...
    public function removeImplementasi(int $askepIndex, int $implIndex): void
    {
        ...
        try {
            $this->withUgdLock(function () use ($askepIndex, $implIndex) {
                $fresh = $this->findDataUGD($this->rjNo) ?? [];
    ...
    public function resetFormEntry(): void
    {
        ...
        $this->dispatch('lov-asuhan-keperawatan.reset', target: 'ugdFormAsuhanKeperawatan');
    }
    ...
    {
        $this->incrementVersion('modal-asuhan-keperawatan-ugd');
        $this->dispatch('refresh-after-ugd.saved', tab: 'asuhan');
        $this->dispatch('toast', type: 'success', message: $msg);
    }

    private function withUgdLock(callable $fn): void
    {
        Cache::lock("ugd:{$this->rjNo}", 10)->block(5, function () use ($fn) {
...
<div class="space-y-4" wire:key="{{ $this->renderKey('modal-asuhan-keperawatan-ugd', [$rjNo ?? 'new']) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                        <livewire:lov.asuhan-keperawatan.lov-asuhan-keperawatan label="Pilih Diagnosis Keperawatan *"
                            target="ugdFormAsuhanKeperawatan" :disabled="$isFormLocked"
                            wire:key="lov-asuhan-keperawatan-{{ $this->renderKey('modal-asuhan-keperawatan-ugd') }}" />
    ...
    @forelse (array_reverse($daftarAskep, true) as $idx => $askep)
        <div wire:key="askep-{{ $idx }}-{{ $this->renderKey('modal-asuhan-keperawatan-ugd') }}"
            ...
            <div>
                @include('pages::transaksi.ugd.emr-ugd.asuhan-keperawatan.riwayat-asuhan-keperawatan', [
                    'askep' => $askep,
                    'idx' => $idx,
            ...
            <div>
                @include(
                    'pages::transaksi.ugd.emr-ugd.asuhan-keperawatan.implementasi-asuhan-keperawatan',
                    [
                        'askep' => $askep,
                        'idx' => $idx,
                        'isFormLocked' => $isFormLocked,
