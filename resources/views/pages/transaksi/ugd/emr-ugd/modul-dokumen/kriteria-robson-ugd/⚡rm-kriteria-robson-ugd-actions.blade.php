<?php
// resources/views/pages/transaksi/ugd/emr-ugd/modul-dokumen/kriteria-robson-ugd/rm-kriteria-robson-ugd-actions.blade.php
// Kriteria Robson (Klasifikasi 10 Kelompok, WHO 2017) — jalur UGD; port dari RI (kriteria-robson-ri).
...
// disimpan ke datadaftarugd_json. Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-kriteria-robson-ugd'];

    /** Key penyimpanan di datadaftarugd_json */
    private string $jsonKey = 'kriteriaRobsonUGD';
    ...
    public function mount(?int $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-kriteria-robson-ugd']);
        $this->resetNewForm();
    ...
    public function openModal(): void
    {
        ...
        $this->incrementVersion('modal-kriteria-robson-ugd');
        $this->layar = 'daftar';
        $this->dispatch('open-modal', name: 'kriteria-robson-ugd');
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: 'kriteria-robson-ugd');
    }
    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Buka kunci Kriteria Robson (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut', 'MR');
            });
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Hapus Kriteria Robson — ' . $createdAt, 'MR');
            });
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'kriteria-robson-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="kriteria-robson-ugd" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
             wire:key="{{ $this->renderKey('modal-kriteria-robson-ugd', [$rjNo ?? 'new']) }}">
            <x-modul-dokumen.header judul="Kriteria Robson"
                ...
                jalur="UGD" :jumlah="count($entriList ?? [])" :readOnly="$isFormLocked">
            ...
            <div class="px-4 pt-2">
                <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                    wire:key="kriteria-robson-ugd-display-pasien-{{ $rjNo }}" />
            </div>
