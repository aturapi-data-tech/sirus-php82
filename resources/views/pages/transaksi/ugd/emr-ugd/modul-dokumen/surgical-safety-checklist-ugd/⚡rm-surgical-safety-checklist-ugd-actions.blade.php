...
<div>
    ...
    <x-modal name="rm-surgical-safety-checklist-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full" wire:key="{{ $this->renderKey('modal-surgical-safety-checklist-ugd', [$rjNo ?? 'new']) }}">
            <x-modul-dokumen.header judul="Surgical Safety Checklist"
                ikon="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
                jalur="UGD" :jumlah="count($surgicalSafetyChecklistList)" :readOnly="$isFormLocked">
                UGD — WHO SSC
                                                ...
                                                <tr @click="open = !open"
                                                    ...
                                                    <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                        <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                            ...
                                                            judulBukaKunci="Buka Kunci Surgical Safety Checklist"
                                                            pesanBukaKunci="KETIGA TTD (Dokter Anestesi, Perawat Instrumen, Operator) akan dicabut & entri kembali menjadi draft untuk dikoreksi. Lanjutkan?"
                                                            konfirmasiHapus="Yakin hapus checklist ini?" />
                                                    </td>
                                                </tr>
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                :lihat="$viewOnly"
                :mengedit="$editingKey">
                Simpan draft dulu, lalu <strong>kunci</strong> lewat tombol <strong>TTD Petugas</strong>.
