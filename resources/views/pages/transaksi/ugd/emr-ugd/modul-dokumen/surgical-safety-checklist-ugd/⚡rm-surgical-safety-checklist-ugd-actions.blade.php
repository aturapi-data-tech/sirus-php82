...
<div>
    ...
    <x-modul-dokumen.kartu judul="Surgical Safety Checklist"
        :jumlah="$entriCount"
        satuan="checklist"
        :nonaktif="$disabled || !$rjNo">
        <x-slot:deskripsi>WHO Surgical Safety Checklist — verifikasi tiga fase: Sign In (sebelum anestesi), Time Out (sebelum insisi), dan Sign Out (sebelum meninggalkan kamar operasi).</x-slot:deskripsi>
        <x-slot:ringkasan>
            @if ($entriCount > 0)
            ...
            <div class="flex-1 px-4 py-4 bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <div class="{{ $this->diForm() ? 'p-6 sm:p-8 bg-canvas border border-hairline shadow-sm rounded-2xl dark:bg-gray-900 dark:border-gray-700' : '' }} space-y-6">
                        ...
                        @endif
                        @unless ($this->diForm())
                            <x-modul-dokumen.tabel-daftar :kolom="['', 'Tanggal', 'Tindakan', 'TTD (3 Pihak)', 'Status' => 'text-center', 'Aksi' => 'text-center']">
