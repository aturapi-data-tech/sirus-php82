...
<div>
    ...
    <x-modal name="rm-form-trf-ugd-ri-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full" wire:key="{{ $this->renderKey('modal-trf-ugd-ri', [$rjNo ?? 'new']) }}">
                                            ...
                                            @if (!$isFormLocked)
                                                <td class="px-3 py-2 text-center">
                                                    <div class="flex items-center justify-center gap-1">
                                                        ...
                                                        @endif
                                                        <x-hapus-button wire:click="removeLevelingDokter('{{ $dok['tglEntry'] }}')" confirm="Hapus dokter ini dari daftar?" />
                                                    </div>
                                                </td>
                                            @endif
                                        </tr>
                        ...
                        <div class="space-y-2">
                            @foreach ($alatList as $index => $alatItem)
                                <div
                                    ...
                                    @if (!$isFormLocked)
                                        <x-hapus-button wire:click="removeAlatTerpasang({{ $index }})" confirm="Hapus alat ini?" />
                                    @endif
                                </div>
                            @endforeach
                        </div>
            ...
            @if (isset($dataDaftarUGD['trfUgd']))
                <div class="sticky bottom-0 z-10 px-6 py-3 bg-canvas border-t border-hairline dark:bg-gray-900 dark:border-gray-700">
                    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                        ...
                        <x-cetak-button wire:click="cetakTrfPasienUgd" class="min-w-[160px]" label="Cetak Form Transfer" />
                        @if (!$isFormLocked)
