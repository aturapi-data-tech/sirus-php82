...
new class extends Component {
    ...
    public function openModal(): void
    {
        ...
        $this->praAnestesiList = $this->dataDaftarUGD['praAnestesiUGD'];
...
<div>
    ...
    <x-modul-dokumen.kartu judul="Pengkajian Pra Anestesi & Pra Sedasi"
        :jumlah="$entriCount"
        satuan="pengkajian"
        :nonaktif="$disabled || !$rjNo">
        <x-slot:deskripsi>Asesmen pra anestesi (PAB 4 / RM 50) oleh dokter anestesi: anamnese, jalan nafas (Mallampati), status fisik ASA, rencana teknik anestesi & analgesia pasca-op. Tiap entri = 1 pengkajian.</x-slot:deskripsi>
        <div class="overflow-x-auto rounded-2xl border border-hairline dark:border-gray-700">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    @forelse (collect($praAnestesiList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                        <tr class="border-b border-hairline dark:border-gray-700">
                            ...
                            <td class="px-3 py-2 text-muted dark:text-gray-400">
                                <x-modul-dokumen.status-ttd :nama="$entry['ttd'] ?? ''" gaya="polos" />
                            </td>
    ...
    <x-modal name="rm-pra-anestesi-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Pengkajian Pra Anestesi & Pra Sedasi"
                ...
                jalur="UGD" :jumlah="count($praAnestesiList)" :readOnly="$isFormLocked">
                PAB 4 / RM 50 — dokter anestesi
                                    ...
                                    <table class="min-w-full text-sm">
                                        <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                            <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                                ...
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">ASA</th>
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Jenis Anestesi</th>
                                                ...
                                                <tr @click="open = !open"
                                                    ...
                                                    <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                                        <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                            ...
                                                            judulBukaKunci="Buka Kunci Pengkajian Pra Anestesi"
                                                            pesanBukaKunci="KEDUA TTD (Dokter Anestesi & Pasien/Keluarga) akan dicabut & entri kembali menjadi Draft — proses TTD diulang dari awal. Lanjutkan?"
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                :lihat="$viewOnly"
                :mengedit="$editingKey">
                Simpan draft dulu; entri otomatis <strong>terkunci</strong> setelah TTD <strong>Dokter Anestesi + Pasien/Keluarga</strong> lengkap.
