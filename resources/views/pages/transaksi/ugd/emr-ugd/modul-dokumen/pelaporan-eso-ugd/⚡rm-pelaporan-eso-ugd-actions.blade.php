                                ...
                                <tbody>
                                    @forelse ($form['obat'] ?? [] as $indexObat => $barisObat)
                                        <tr wire:key="eso-obat-{{ $rjNo ?? 'new' }}-{{ $indexObat }}">
                                            ...
                                            <td class="ds-c">
                                                @unless ($formReadOnly)
                                                    <x-hapus-button :action="'removeBarisObat(' . $indexObat . ')'" title="Hapus Obat" :message="'Yakin hapus ' . ($barisObat['namaObat'] ?? 'obat ini') . ' dari daftar?'" />
                                                @else
                                ...
                                @forelse (collect($dataDaftarUGD['pelaporanEsoUGD'] ?? [])->sortByDesc(fn($entri) => strtotime(strtr((data_get($entri, 'form.tglLaporan') ?: ($entri['created_at'] ?? '')), '/', '-')))->values()->all() as $indexEntri => $entri)
                                    @php
                                        ...
                                        $obatDicurigai = collect((array) data_get($entri, 'form.obat', []))
                                            ...
                                            ->count();
                                    @endphp
                                    <tbody wire:key="eso-entri-{{ $rjNo ?? 'new' }}-{{ $idEntri ?? $indexEntri }}" x-data="{ open: false }" class="border-b border-hairline dark:border-gray-700">
