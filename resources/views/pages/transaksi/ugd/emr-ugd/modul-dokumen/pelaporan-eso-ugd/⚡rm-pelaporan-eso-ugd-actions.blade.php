        ...
        {{-- PRATINJAU ENTRI DI KARTU — ringkasan entri terbaru, tanpa perlu membuka modal --}}
            <div class="mt-3 overflow-x-auto rounded-2xl border border-hairline dark:border-gray-700">
                <table class="ds-table">
                    ...
                    <tbody>
                        @forelse (array_slice(collect($dataDaftarUGD['pelaporanEsoUGD'] ?? [])->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all(), 0, 3) as $indexEntri => $entri)
                            @php
