<?php
// resources/views/pages/transaksi/ugd/emr-ugd/modul-dokumen/pelaporan-eso-ugd/rm-pelaporan-eso-ugd-actions.blade.php
//
// RM 37 — Formulir Pelaporan Efek Samping Obat (ESO), UGD.
// Port dari versi Rawat Inap; beda jalurnya hanya sumber prefill (lihat di bawah).
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-pelaporan-eso-ugd'];
    ...
    public function mount(?int $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-pelaporan-eso-ugd']);
    ...
    {
        $this->dispatch('close-modal', name: "rm-pelaporan-eso-ugd-{$this->rjNo}");
    }
    ...
    private function prefillDariEmr(): void
    {
        ...
        // UGD tidak punya pengkajianAwalPasienRawatInap. Padanan terdekat penyakit
    ...
    private function persistEntry(string $id, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $id, $logVerb) {
            ...
            if (!isset($fresh['pelaporanEsoUGD']) || !is_array($fresh['pelaporanEsoUGD'])) {
                $fresh['pelaporanEsoUGD'] = [];
            }

            $daftarEntri = $fresh['pelaporanEsoUGD'];
    ...
    public function cetak(string $id)
    {
        ...
        try {
            ...
            $pdf = Pdf::loadView('pages.components.modul-dokumen.ugd.pelaporan-eso.cetak-pelaporan-eso-print', ['data' => $data])->setPaper('A4');

            $this->dispatch('toast', type: 'success', message: 'Berhasil mencetak Pelaporan ESO.');
            return response()->streamDownload(fn() => print $pdf->output(), 'pelaporan-eso-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
...
<div>
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 space-y-2">
                <div class="flex items-center gap-2">
                    <h3 class="text-base font-semibold text-ink dark:text-gray-200">Pelaporan Efek Samping Obat</h3>
                ...
                <p class="text-base text-muted dark:text-gray-400">
                    Formulir kuning MESO: data penderita, manifestasi efek samping, daftar obat yang
                    dicurigai, lalu ditandatangani pelapor. Bentuk kolom mengikuti Form Kuning BPOM 2026
                    supaya bisa langsung dilaporkan ke e-MESO.
                </p>
            </div>
    ...
    <x-modal name="rm-pelaporan-eso-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="relative flex flex-col min-h-[calc(100vh-8rem)]">
            ...
            <x-icon-button color="gray" type="button" wire:click="closeModal" class="absolute z-20 top-3 right-3 bg-canvas/90 backdrop-blur-sm">
            ...
            <div class="flex-1 p-4 space-y-4 sm:p-6"
                wire:key="{{ $this->renderKey('modal-pelaporan-eso-ugd', [$rjNo ?? 'new']) }}">
                            ...
                            <table class="ds-table">
                                ...
                                <tbody>
                                    @forelse ($form['obat'] ?? [] as $indexObat => $barisObat)
                                        <tr wire:key="eso-obat-{{ $rjNo ?? 'new' }}-{{ $indexObat }}">
                                            @php
                ...
                <x-border-form title="Laporan ESO Tersimpan" align="start" bgcolor="bg-surface-soft">
                    <div class="mt-3 overflow-x-auto border bg-canvas rounded-2xl border-hairline dark:border-gray-700">
                        <table class="ds-table">
                            ...
                            <tbody>
                                @forelse ($dataDaftarUGD['pelaporanEsoUGD'] ?? [] as $indexEntri => $entri)
                                    @php
                                        ...
                                        $obatDicurigai = collect((array) data_get($entri, 'form.obat', []))
                                            ...
                                            ->count();
                                    @endphp
                                    <tr wire:key="eso-entri-{{ $rjNo ?? 'new' }}-{{ $idEntri ?? $indexEntri }}">
