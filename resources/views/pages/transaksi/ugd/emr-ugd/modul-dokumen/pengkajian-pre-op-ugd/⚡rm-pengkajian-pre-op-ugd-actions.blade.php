            ...
            foreach (['ttdPerawatRuangan', 'ttdPerawatKamarBedah', 'ttdDokterOperator'] as $ttdField) {
                ...
                if ($code) {
                    ...
                    if (!empty($path) && file_exists(TtdUser::pathBerkas($path))) {
                        $ttdPaths[$ttdField . 'Path'] = TtdUser::pathBerkas($path);
                    }
                }
            }
    ...
    <x-modal name="rm-pengkajian-pre-op-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Pengkajian Pre Operasi"
                ...
                jalur="UGD" :jumlah="count($preOpList)" :readOnly="$isFormLocked">
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                :lihat="$viewOnly"
                :mengedit="$editingKey">
                Simpan draft dulu; entri otomatis <strong>terkunci</strong> setelah TTD <strong>Perawat Ruangan + Perawat Kamar Bedah + Dokter Operator</strong> lengkap.
