<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/site-marking-ri/rm-site-marking-ugd-actions.blade.php
...
// disimpan ke datadaftarri_json['siteMarkingUGD']. Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-site-marking-ugd'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'siteMarkingUGD';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-site-marking-ugd']);
    ...
    public function openModal(): void
    {
        ...
        $this->markingList = $this->dataDaftarUGD[$this->jsonKey];
        $this->isFormLocked = $this->checkEmrUGDStatus($this->rjNo) || $this->disabled;
        $this->incrementVersion('modal-site-marking-ugd');

        $this->dispatch('open-modal', name: "rm-site-marking-ugd-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-site-marking-ugd-{$this->rjNo}");
    }
    ...
    private function persistEntry(string $key, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $key, $logVerb) {
            ...
            $this->appendAdminLogUGD((int) $this->rjNo, $logVerb . ' Penandaan Lokasi Operasi — ' . $lokasi . ' (' . $key . ')', 'MR');
        });
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'site-marking-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Hapus Penandaan Lokasi Operasi — ' . $createdAt, 'MR');
            });
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="rm-site-marking-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            wire:key="{{ $this->renderKey('modal-site-marking-ugd', [$rjNo ?? 'new']) }}">
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                        wire:key="sm-ugd-display-pasien-{{ $rjNo ?? 'init' }}" />
