...
<div>
    ...
    <x-modul-dokumen.kartu judul="Penolakan Pengobatan / Obat Tertentu"
        :jumlah="$poCount"
        satuan="surat"
        :nonaktif="$disabled || !$rjNo">
        <x-slot:deskripsi>Surat pernyataan pasien/keluarga yang menolak pemberian pengobatan/obat tertentu setelah mendapat penjelasan dokter/petugas. Dapat lebih dari satu surat.</x-slot:deskripsi>
        <div class="overflow-x-auto rounded-2xl border border-hairline dark:border-gray-700">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    @forelse (collect($penolakanList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $po)
    ...
    <x-modal name="rm-penolakan-obat-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Surat Pernyataan Penolakan Pengobatan / Obat Tertentu"
            ...
            <div class="flex-1 px-4 py-4 bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <div
                        ...
                        @endif
                        @unless ($this->diForm())
                            <x-modul-dokumen.tabel-daftar :kolom="['', 'Pembuat Pernyataan', 'Obat yang Ditolak', 'Tanggal Dibuat', 'Petugas RS', 'Status' => 'text-center', 'Aksi' => 'text-center']">
                                            ...
                                            <tr @click="open = !open"
                                                ...
                                                <td class="px-4 py-3 align-middle text-center whitespace-nowrap" @click.stop>
                                                    <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                        ...
                                                        judulBukaKunci="Buka Kunci Surat Penolakan Obat"
