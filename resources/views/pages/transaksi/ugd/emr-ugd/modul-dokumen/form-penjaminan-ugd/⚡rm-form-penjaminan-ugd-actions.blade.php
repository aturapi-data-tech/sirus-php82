        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div class="flex-1 min-w-0 space-y-2">
                ...
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    ...
                    @endif
                    <p class="flex-1 min-w-0 hidden truncate text-sm text-muted sm:block dark:text-gray-400">Pernyataan kepemilikan kartu penjaminan biaya &amp; orientasi kamar pasien UGD.</p>
                </div>
            </div>
                                        ...
                                        @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                            ...
                                            @if ($isFinal)
                                                @can('dokumen.bukaKunci')
                                                    <x-confirm-button variant="warning-soft" action="bukaKunci('{{ $rowKey }}')" title="Buka Kunci Form Penjaminan"
                                            ...
                                            @endif
                                            @can('dokumen.hapus')
                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus form penjaminan ini? Data yang sudah ditandatangani akan dihapus." />
                                            @endcan
                                            </div>
                                        @endif
                                    </div>
            ...
            <div class="sticky bottom-0 z-10 px-6 py-4 bg-canvas border-t border-hairline dark:bg-gray-900 dark:border-gray-700">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-3">
                        <x-secondary-button type="button" wire:click="kembaliKeDaftar" class="min-w-[110px] justify-center">Kembali ke Daftar</x-secondary-button>
