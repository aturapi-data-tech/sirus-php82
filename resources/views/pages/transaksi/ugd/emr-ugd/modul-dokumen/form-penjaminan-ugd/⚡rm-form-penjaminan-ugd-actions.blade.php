...
new class extends Component {
    ...
    /* ===============================
     ...
     =============================== */
    public function bukaKunci(string $signaturePembuatDate): void
    {
        ...
        try {
            DB::transaction(function () use ($signaturePembuatDate) {
                $this->lockUGDRow($this->rjNo);
                $data = $this->findDataUGD($this->rjNo);
                $list = is_array($data['formPenjaminanOrientasiKamar'] ?? null) ? $data['formPenjaminanOrientasiKamar'] : [];
                $index = collect($list)->search(fn($item) => ($item['signaturePembuatDate'] ?? '') === $signaturePembuatDate);
                if ($index === false) {
                    throw new \RuntimeException('Entri tidak ditemukan.');
                }
                $list[$index]['finalized'] = false;
                $list[$index]['namaPetugas'] = '';
                $list[$index]['kodePetugas'] = '';
...
<div>
    ...
    <x-modul-dokumen.kartu judul="Formulir Penjaminan & Orientasi Kamar"
        :jumlah="$penjaminanCount"
        satuan="tersimpan"
        tombol="Buka Form Penjaminan"
        :nonaktif="!$rjNo">
        <x-slot:deskripsi>Pernyataan kepemilikan kartu penjaminan biaya &amp; orientasi kamar pasien UGD.</x-slot:deskripsi>
                    ...
                    @forelse (collect($listForm)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $ic)
                        ...
                        <tr class="border-b border-hairline dark:border-gray-700">
                            ...
                            <td class="px-3 py-2 text-muted dark:text-gray-400">
                                <x-modul-dokumen.status-ttd :nama="$ic['namaPetugas'] ?? ''" gaya="polos" />
                            </td>
                            <td class="px-3 py-2 text-center">
                                <x-modul-dokumen.status-entri :final="$this->entryIsFinal($ic)" />
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <x-modal name="rm-form-penjaminan-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full" wire:key="{{ $this->renderKey('modal-form-penjaminan', [$rjNo ?? 'new']) }}">
        ...
        @if ($isFormLocked)
            <x-modul-dokumen.banner jenis="terkunci" class="mt-4 mx-4" />
        @endif

        @if ($viewOnly)
            <x-modul-dokumen.banner jenis="lihat" class="mt-4 mx-4" />
        @elseif ($editingKey && !$isFormLocked)
            <x-modul-dokumen.banner jenis="lanjut" class="mt-4 mx-4" />
        @endif
        ...
        @endif
        @unless ($this->diForm())
            <x-modul-dokumen.tabel-daftar class="mx-4 mt-4" :kolom="['', 'Tanggal', 'Nama Pembuat', 'Jenis Penjamin', 'Petugas', 'Status' => 'text-center', 'Aksi' => 'text-center']">
                    @forelse (collect($listForm)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                        ...
                        <tbody x-data="{ open: false }" class="border-b border-hairline dark:border-gray-700">
                            <tr @click="open = !open"
                                ...
                                <td class="px-4 py-3 align-middle text-muted dark:text-gray-300">
                                    <x-modul-dokumen.status-ttd :nama="$entry['namaPetugas'] ?? ''" />
                                </td>
                                ...
                                <td class="px-4 py-3 align-middle text-center whitespace-nowrap" @click.stop>
                                    <x-modul-dokumen.aksi-entri kunci="{{ $rowKey }}" :final="$isFinal" :terkunci="$isFormLocked"
                                        ...
                                        judulBukaKunci="Buka Kunci Form Penjaminan"
                                        konfirmasiHapus="Yakin hapus form penjaminan ini? Data yang sudah ditandatangani akan dihapus." />
                                </td>
                            </tr>
                                ...
                                <td colspan="7" class="px-4 py-4 bg-surface-soft/60 dark:bg-gray-950/30">
                                    <dl class="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                                        ...
                                        <div>
                                            ...
                                            <dd class="mt-0.5">
                                                <x-modul-dokumen.status-ttd :sudah="!empty($entry['signaturePembuat'])" :waktu="$entry['signaturePembuatDate'] ?? '-'" gaya="biasa" />
                                            </dd>
                                        </div>
                                        <div>
                                            <dt class="text-xs font-semibold tracking-wide uppercase text-muted-soft">TTD Saksi Keluarga</dt>
                                            <dd class="mt-0.5">
                                                <x-modul-dokumen.status-ttd :sudah="!empty($entry['signatureSaksiKeluarga'])" :waktu="$entry['signatureSaksiKeluargaDate'] ?? '-'" gaya="biasa" />
                                            </dd>
                                        </div>
                                        <div>
        ...
        @endunless
            ...
            <x-modul-dokumen.footer :formulir="$this->diForm()" :terkunci="$isFormLocked"
                ...
                :mengedit="$editingKey">
                Simpan draft dulu, lalu <strong>kunci</strong> lewat tombol <strong>TTD Petugas &amp; Kunci</strong> di kolom Petugas Rumah Sakit.
