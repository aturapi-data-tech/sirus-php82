...
new class extends Component {
    ...
    /**
     * IRISAN dokumen: cabang `pengkajianAkhirHayatUGD` + beberapa nilai rekam medis yang dipotret
     * sebagai isian awal formulir.
     */
    public array $daftarAkhirHayat = [];

    /** Dokumen dibaca sebagai variabel LOKAL; hanya irisan + skalar yang disimpan. */
    private function muatDariDokumen(array $data): void
    {
        $this->daftarAkhirHayat = $data['pengkajianAkhirHayatUGD'] ?? [];
    ...
    {
        $diagnosa = $this->diagnosisFreeText;
    ...
    public function setKeluargaSignature(string $dataUrl): void
    {
        ...
        $this->keluargaSignature = TtdPasien::simpan($dataUrl, $this->regNo);   // gambar ke RSTXN_TTDS; di sini cukup referensi "TTD:<no>"
    ...
    public function setSaksiSignature(string $dataUrl): void
    {
        ...
        $this->saksiSignature = TtdPasien::simpan($dataUrl, $this->regNo);   // gambar ke RSTXN_TTDS; di sini cukup referensi "TTD:<no>"
        $this->incrementVersion('modal-akhir-hayat-ugd');
    }
    ...
    private function buildEntry(string $id, bool $finalized): array
    {
        ...
        $existing = collect($this->daftarAkhirHayat ?? [])->firstWhere('id', $id);
    ...
    private function persistEntry(string $id, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $id, $logVerb) {
            ...
            $this->daftarAkhirHayat = $fresh['pengkajianAkhirHayatUGD'] ?? [];
    ...
    public function editEntry(string $id): void
    {
        ...
        $entri = collect($this->daftarAkhirHayat ?? [])->firstWhere('id', $id);
        if (!$entri) {
    ...
    {
        $entry = collect($this->daftarAkhirHayat ?? [])->firstWhere('id', $id);
        if (!$entry) {
    ...
    {
        $this->form = $this->defaultForm();
        $this->form['ttd']['keluargaNama'] = $this->regName;
        $this->prefillHeader();
        $this->keluargaSignature = '';
        $this->saksiSignature = '';
...
<div>
    {{-- ══ RINGKASAN + TOMBOL ══ --}}
    @php $akhirHayatCount = count($daftarAkhirHayat ?? []); @endphp
    <x-modul-dokumen.kartu judul="Pengkajian Akhir Hayat"
        ...
        @php $list = $daftarAkhirHayat ?? []; @endphp
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-surface-card dark:bg-gray-800">
                        <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Tanggal</th>
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Jenis</th>
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Penanda Tangan</th>
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Petugas</th>
                            <th class="whitespace-nowrap px-3 py-2 text-sm font-medium text-center text-muted dark:text-gray-400 border-b border-hairline dark:border-gray-700">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    <div class="flex flex-wrap items-end gap-3 p-4 bg-canvas border border-hairline rounded-2xl dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <div class="pb-1">
                            @php
                                ...
                                $jenisAktif = $jenisTersimpan ?: (count($daftarAkhirHayat ?? []) > 0 ? 'ulang' : 'awal');
                            @endphp
                                ...
                                <tbody wire:key="akhir-hayat-{{ $id ?: $loop->index }}"
                                    ...
                                    <tr @click="open = !open"
                                        ...
                                        <td class="px-4 py-3 text-center align-middle whitespace-nowrap" @click.stop>
                                            <x-modul-dokumen.aksi-entri kunci="{{ $id }}" :final="$isFinal" :terkunci="$isFormLocked"
                                                :bisaDiubah="filled($id)"
                                                hapus="removeEntry"
                                                judulLihat="Lihat entri terkunci"
                                                judulBukaKunci="Buka Kunci Pengkajian Akhir Hayat"
                                                pesanBukaKunci="TTD petugas akan dicabut & entri kembali menjadi draft untuk dikoreksi. TTD pasien/keluarga & saksi tetap. Lanjutkan?"
                                                konfirmasiHapus="Hapus pengkajian ini?" />
                                        </td>
                                    </tr>
