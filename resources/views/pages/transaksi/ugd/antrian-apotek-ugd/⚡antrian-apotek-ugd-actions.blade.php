...
new class extends Component {
    ...
    public function bukaKunciTelaah(string $bagian): void
    {
        ...
        try {
            DB::transaction(function () use ($bagian, $label) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Buka kunci ' . $label . ' apotek — TTD ' . ($pj['userLog'] ?? '-') . ' (' . ($pj['userLogDate'] ?? '-') . ') dicabut oleh ' . (auth()->user()->myuser_name ?? auth()->user()->name ?? '-'), 'MR');
            });
            $this->incrementVersion('modal-telaah-apotek-ugd');
    ...
    public function saveTelaahResep(): void
    {
        ...
        try {
            DB::transaction(function () {
                ...
                $data['telaahObat'] = array_replace($this->defaultTelaahObat(), $this->dataDaftarUGD['telaahObat'] ?? []); // satu tampilan, satu simpan
    ...
    public function saveTelaahObat(): void
    {
        ...
        try {
            DB::transaction(function () {
                ...
                $data['telaahResep'] = array_replace($this->defaultTelaahResep(), $this->dataDaftarUGD['telaahResep'] ?? []); // satu tampilan, satu simpan
    ...
    {
        $isianLayar = $this->dataDaftarUGD[$nodeDipertahankan] ?? null;
        $this->dataDaftarUGD = $data;
        if (is_array($isianLayar)) {
            $this->dataDaftarUGD[$nodeDipertahankan] = $isianLayar;
        }
        $this->lengkapiDefaultTelaah();
    }
...
<div>
    <x-modal name="telaah-apotek-ugd" size="full" height="full" focusable>
        <div wire:key="{{ $this->renderKey('modal-telaah-apotek-ugd', [$rjNo ?? 'new']) }}">
            ...
            @php
                $ttdResep = $dataDaftarUGD['telaahResep']['penanggungJawab'] ?? null;
                $ttdObat = $dataDaftarUGD['telaahObat']['penanggungJawab'] ?? null;
            @endphp
            <div
