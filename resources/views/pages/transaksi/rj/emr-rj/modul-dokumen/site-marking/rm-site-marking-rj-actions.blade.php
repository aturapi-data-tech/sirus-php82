<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/site-marking-ri/rm-site-marking-rj-actions.blade.php
...
// disimpan ke datadaftarri_json['siteMarkingRJ']. Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-site-marking-rj'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'siteMarkingRJ';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-site-marking-rj']);
    ...
    public function openModal(): void
    {
        ...
        $this->markingList = $this->dataDaftarPoliRJ[$this->jsonKey];
        $this->isFormLocked = $this->checkEmrRJStatus($this->rjNo) || $this->disabled;
        $this->incrementVersion('modal-site-marking-rj');

        $this->dispatch('open-modal', name: "rm-site-marking-rj-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-site-marking-rj-{$this->rjNo}");
    }
    ...
    private function persistEntry(string $key, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $key, $logVerb) {
            ...
            $this->appendAdminLogRJ((int) $this->rjNo, $logVerb . ' Penandaan Lokasi Operasi — ' . $lokasi . ' (' . $key . ')', 'MR');
        });
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'site-marking-rj-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogRJ((int) $this->rjNo, 'Hapus Penandaan Lokasi Operasi — ' . $createdAt, 'MR');
            });
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="rm-site-marking-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            wire:key="{{ $this->renderKey('modal-site-marking-rj', [$rjNo ?? 'new']) }}">
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.rj.display-pasien-rj.display-pasien-rj :rjNo="$rjNo"
                        wire:key="sm-rj-display-pasien-{{ $rjNo ?? 'init' }}" />
