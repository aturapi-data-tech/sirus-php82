    ...
    public function bukaKunci(string $signatureDate): void
    {
        ...
        try {
            DB::transaction(function () use ($signatureDate) {
                $this->lockRJRow($this->rjNo);
                $data = $this->findDataRJ($this->rjNo);
                $list = is_array($data['penundaanPelayananRJ'] ?? null) ? $data['penundaanPelayananRJ'] : [];
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-baseline flex-1 gap-2 min-w-0">
                    ...
                    @endif
                    <x-deskripsi-ringkas class="hidden sm:flex text-sm">Formulir pemberitahuan kepada pasien/keluarga atas penundaan atau kelambatan pelayanan beserta alasan dan alternatif yang ditawarkan. Dapat lebih dari satu catatan.</x-deskripsi-ringkas>
                </div>
                ...
                <div class="relative flex items-center gap-3 min-w-0">
                    <div class="flex items-center flex-1 gap-3 min-w-0">
                        <div class="flex items-center gap-2.5 min-w-0">
                            <div class="flex items-center justify-center w-7 h-7 rounded-lg shrink-0 bg-amber-500/10">
                        ...
                        <div class="flex items-center gap-1.5 ml-auto shrink-0">
                            <x-badge class="shrink-0 whitespace-nowrap" variant="success">Rawat Jalan</x-badge>
                            @if (count($penundaanList) > 0)
                                <x-badge class="shrink-0 whitespace-nowrap" variant="info">{{ count($penundaanList) }} tersimpan</x-badge>
                            @endif
                            @if ($isFormLocked)
                                                        ...
                                                        @if (!$isFormLocked && (auth()->user()?->can('dokumen.bukaKunci') || auth()->user()?->can('dokumen.hapus')))
                                                            ...
                                                            @if ($isFinal)
                                                                @can('dokumen.bukaKunci')
                                                                    <x-confirm-button variant="warning-soft" action="bukaKunci('{{ $rowKey }}')" title="Buka Kunci Pemberitahuan Penundaan Pelayanan"
                                                            ...
                                                            @endif
                                                            @can('dokumen.hapus')
                                                            <x-hapus-button wire:click.prevent="hapus('{{ $rowKey }}')" confirm="Yakin hapus pemberitahuan ini?" />
                                                            @endcan
                                                            </div>
                                                        @endif
                                                    </div>
