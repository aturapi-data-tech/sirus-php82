...
<div>
    ...
    <x-modal name="rm-pra-induksi-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full" wire:key="{{ $this->renderKey('modal-pra-induksi-rj', [$rjNo ?? 'new']) }}">
            <x-modul-dokumen.header judul="Asesmen Pra Induksi"
                ...
                PAB 6 / RM 50.a — sesaat sebelum induksi
                                                ...
                                                <tbody>
                                                    @forelse ($newForm['obatPreMedikasi'] ?? [] as $preMedikasiIndex => $preMedikasiItem)
                                                        <tr wire:key="pre-med-row-{{ $preMedikasiIndex }}">
                                                            ...
                                                            <td class="ds-c">
                                                                @if (!$formReadOnly)
                                                                    <x-hapus-button :action="'removePreMedikasi(' . $preMedikasiIndex . ')'" title="Hapus Baris" :message="'Yakin hapus ' . ($preMedikasiItem['obat'] ?? 'baris ini') . ' dari daftar?'" />
                                                                @else
                                    ...
                                    <table class="min-w-full text-base">
                                        <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                                            <tr class="text-xs font-semibold tracking-wide text-left text-muted uppercase dark:text-gray-300">
                                                ...
                                                <th class="whitespace-nowrap px-4 py-3 border-b bg-surface-card dark:bg-gray-800">Rencana</th>
                                        ...
                                        @forelse (collect($praInduksiList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $entry)
                                            @php
                                                $isFinal = $this->entryIsFinal($entry);
                                                $rowKey = $entry['createdAt'] ?? '';
                                            @endphp
