...
new class extends Component {
    ...
    public function openModal(): void
    {
        ...
        $this->consentList = is_array($data['informConsentPasienRJ'] ?? null) ? $data['informConsentPasienRJ'] : [];
        $this->regName = $data['regName'] ?? null;
...
<div>
    ...
    <x-modul-dokumen.kartu judul="Inform Consent"
        :jumlah="$icCount"
        satuan="tindakan"
        tombol="Buka Inform Consent"
        :nonaktif="$disabled || !$rjNo">
        <x-slot:deskripsi>Persetujuan tindakan medis per-tindakan: tujuan, risiko, alternatif, serta tanda tangan pasien/wali, dokter penjelas, dan saksi.</x-slot:deskripsi>
                    ...
                    @forelse (collect($consentList)->sortByDesc(fn($entri) => strtotime(strtr(($entri['tanggal'] ?? '') ?: ($entri['createdAt'] ?? ''), '/', '-')))->values()->all() as $ic)
                        <tr class="border-b border-hairline dark:border-gray-700">
                            ...
                            <td class="px-3 py-2 text-muted dark:text-gray-400">
                                <x-modul-dokumen.status-ttd :nama="$ic['dokter'] ?? ''" gaya="polos" />
                            </td>
    ...
    <x-modal name="rm-inform-consent-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full"
            ...
            <x-modul-dokumen.header judul="Inform Consent"
                ...
                jalur="RJ" :jumlah="$icCount" :readOnly="$isFormLocked">
            ...
            <div class="flex-1 px-4 py-4 bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <div
                        class="{{ $this->diForm() ? 'p-4 bg-canvas border border-hairline shadow-sm rounded-2xl dark:bg-gray-900 dark:border-gray-700' : '' }} space-y-4">

                        @php $formReadOnly = $isFormLocked || $viewOnly; @endphp

                        @if ($isFormLocked)
                            <x-modul-dokumen.banner jenis="terkunci" class="mb-4" />
                        @endif
                        ...
                        @endif
                        @unless ($this->diForm())
                            <x-modul-dokumen.tabel-daftar :kolom="['', 'Tindakan', 'Tanggal Dibuat', 'Pemberi Informasi', 'Persetujuan' => 'text-center', 'Status' => 'text-center', 'Aksi' => 'text-center']">
