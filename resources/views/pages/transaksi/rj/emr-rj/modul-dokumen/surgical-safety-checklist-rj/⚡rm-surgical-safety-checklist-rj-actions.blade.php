...
<div>
    ...
    <x-modal name="rm-surgical-safety-checklist-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-full" wire:key="{{ $this->renderKey('modal-surgical-safety-checklist-rj', [$rjNo ?? 'new']) }}">
            <x-modul-dokumen.header judul="Surgical Safety Checklist"
                ikon="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
                jalur="RJ" :jumlah="count($surgicalSafetyChecklistList)" :readOnly="$isFormLocked">
                Rawat Jalan — WHO SSC
