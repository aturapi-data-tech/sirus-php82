<?php
// resources/views/pages/transaksi/rj/emr-rj/asuhan-keperawatan/rm-asuhan-keperawatan-rj-actions.blade.php
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-asuhan-keperawatan-rj'];

    /**
     * rjNo datang lewat PROP dari induk (anak lahir di dalam @if($rjNo)), bukan lagi
    ...
    {
        $this->registerAreas(['modal-asuhan-keperawatan-rj']);
    ...
    #[On('open-rm-asuhan-keperawatan-rj')]
    ...
    #[On('lov.selected.rjFormAsuhanKeperawatan')]
    ...
    public function removeAsuhanKeperawatan(int $index): void
    {
        ...
        try {
            $this->withRjLock(function () use ($index) {
                $fresh = $this->findDataRJ($this->rjNo) ?? [];
    ...
    /* ── IMPLEMENTASI (tindakan SIKI + skor SLKI, disimpan di dalam askep) ── */
    ...
    public function addImplementasi(): void
    {
        ...
        try {
            ...
            $this->withRjLock(function () use ($idx) {
                $fresh = $this->findDataRJ($this->rjNo) ?? [];
                ...
                // Simpan di dalam askep. TANPA sinkron ke CPPT: RJ memang tak punya
                // CPPT/SOAP — implementasinya berhenti di sini, tidak dilempar ke
                // timeline profesi lain seperti di RI.
    ...
    public function removeImplementasi(int $askepIndex, int $implIndex): void
    {
        ...
        try {
            $this->withRjLock(function () use ($askepIndex, $implIndex) {
                $fresh = $this->findDataRJ($this->rjNo) ?? [];
    ...
    public function resetFormEntry(): void
    {
        ...
        $this->dispatch('lov-asuhan-keperawatan.reset', target: 'rjFormAsuhanKeperawatan');
    }
    ...
    {
        $this->incrementVersion('modal-asuhan-keperawatan-rj');
        $this->dispatch('refresh-after-rj.saved', tab: 'asuhan');
        $this->dispatch('toast', type: 'success', message: $msg);
    }

    private function withRjLock(callable $fn): void
    {
        Cache::lock("rj:{$this->rjNo}", 10)->block(5, function () use ($fn) {
...
<div class="space-y-4" wire:key="{{ $this->renderKey('modal-asuhan-keperawatan-rj', [$rjNo ?? 'new']) }}">
        ...
        <div x-show="showGuide" x-transition class="mt-3 text-sm text-blue-900 dark:text-blue-200 space-y-3">
            ...
            <div class="p-2.5 rounded-lg bg-blue-100 dark:bg-blue-900/30 space-y-1">
                ...
                <ul class="ml-4 list-disc space-y-0.5">
                    ...
                    <li>Lewat tombol <strong>Implementasi</strong> pada tiap diagnosis, perawat mencentang
                        <strong>tindakan SIKI</strong> yang sudah dilakukan dan memberi
                        <strong>skor evaluasi SLKI (1-5)</strong> untuk menilai capaian luaran.</li>
                </ul>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                        <livewire:lov.asuhan-keperawatan.lov-asuhan-keperawatan label="Pilih Diagnosis Keperawatan *"
                            target="rjFormAsuhanKeperawatan" :disabled="$isFormLocked"
                            wire:key="lov-asuhan-keperawatan-{{ $this->renderKey('modal-asuhan-keperawatan-rj') }}" />
    ...
    @forelse (array_reverse($daftarAskep, true) as $idx => $askep)
        <div wire:key="askep-{{ $idx }}-{{ $this->renderKey('modal-asuhan-keperawatan-rj') }}"
            ...
            <div>
                @include('pages::transaksi.rj.emr-rj.asuhan-keperawatan.riwayat-asuhan-keperawatan', [
                    'askep' => $askep,
                    'idx' => $idx,
            ...
            <div>
                @include(
                    'pages::transaksi.rj.emr-rj.asuhan-keperawatan.implementasi-asuhan-keperawatan',
                    [
                        'askep' => $askep,
                        'idx' => $idx,
                        'isFormLocked' => $isFormLocked,
