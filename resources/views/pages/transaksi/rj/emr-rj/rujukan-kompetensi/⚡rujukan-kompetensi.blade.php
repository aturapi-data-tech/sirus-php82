...
new class extends Component {
    ...
    public array $kriteriaItems = [['linkId' => '3216', 'text' => 'Terapy/Pengobatan', 'type' => 'boolean', 'answer' => false], ['linkId' => '3215', 'text' => 'Tindakan Medis', 'type' => 'string', 'answer' => ''], ['linkId' => '3214', 'text' => 'Upaya Diagnosis', 'type' => 'boolean', 'answer' => false]];
    ...
    // Progressive step state sesuai PDF BPJS:
    //   1 = Data Rujukan + Satu Sehat       (sebelum GetKriteriaRujukan)
    //   2 = Kriteria & Jejaring Wilayah     (setelah GetKriteriaRujukan)
    //   3 = Cari & Pilih Faskes Tujuan      (setelah GetFaskesRujukan)
    //   4 = Hasil Rujukan                   (setelah POST /Rujukan/Insert)
    public int $step = 1;
    ...
    public function muatKriteria(bool $silent = false): void
    {
        if (!$this->kodeDiagnosa || !$this->kodeFaskesSatuSehat) {
            if (!$silent) {
                $this->dispatch('toast', type: 'error', message: 'Diagnosa & Kode Faskes Satu Sehat harus terisi dulu.');
            }
            return;
        }
        ...
        try {
            $res = self::sisrute_get_kriteria_rujukan($this->kodeDiagnosa, $this->kodeFaskesSatuSehat)->getOriginalContent();
            dd($res);

            if (($res['metadata']['code'] ?? 0) != 200) {
                if (!$silent) {
                    $this->dispatch('toast', type: 'error', message: 'Gagal muat kriteria: ' . ($res['metadata']['message'] ?? '-'));
                }
                return;
            }
            ...
            $items = $payload['kriteriaRujukan']['item'] ?? ($payload['item'] ?? ($payload['kriteria'] ?? ($payload['list'] ?? [])));

            if (empty($items)) {
                if (!$silent) {
                    $this->dispatch('toast', type: 'warning', message: 'Server tidak kasih kriteria — pakai default.');
                }
                return;
            }

            // Normalize struktur kriteria
            $this->kriteriaItems = array_values(
            ...
            if (!empty($wilayah)) {
                if (!$this->kodePropinsi) {
                    $this->kodePropinsi = (string) ($wilayah['kodePropinsi'] ?? '');
                }
                if (!$this->namaPropinsi) {
                    $this->namaPropinsi = (string) ($wilayah['namaPropinsi'] ?? '');
                }
                if (!$this->kodeKabupaten) {
                    $this->kodeKabupaten = (string) ($wilayah['kodeKabupaten'] ?? '');
                }
                if (!$this->namaKabupaten) {
                    $this->namaKabupaten = (string) ($wilayah['namaKabupaten'] ?? '');
                }
            }
            ...
            if (!$silent) {
                $this->dispatch('toast', type: 'success', message: count($this->kriteriaItems) . ' kriteria dimuat dari Satu Sehat Rujukan.');
            }
        } catch (\Throwable $e) {
            if (!$silent) {
    ...
    private function loadContext(): void
    {
        ...
        $regNo = $data['regNo'] ?? '';
        $this->idPasienSatuSehat = $regNo ? (string) (DB::table('rsmst_pasiens')->where('reg_no', $regNo)->value('patient_uuid') ?? '') : '';

        $drId = $data['drId'] ?? '';
        $this->kdDokterSatuSehat = $drId ? (string) (DB::table('rsmst_doctors')->where('dr_id', $drId)->value('dr_uuid') ?? '') : '';
        ...
        $this->missingFields = [];
        if (!$this->noSep) {
            $this->missingFields[] = 'No. SEP — pasang SEP VClaim dulu.';
        }
        if (!$this->kodeFaskesSatuSehat) {
            $this->missingFields[] = 'Kode Faskes Satu Sehat — set env SATUSEHAT_ORGANIZATION_ID.';
        }
        if (!$this->idPasienSatuSehat) {
            $this->missingFields[] = 'ID Pasien Satu Sehat — patient_uuid kosong di rsmst_pasiens (sync dulu via Satu Sehat Patient API).';
        }
        if (!$this->kdDokterSatuSehat) {
            $this->missingFields[] = 'Kd Dokter Satu Sehat — dr_uuid kosong di rsmst_doctors untuk dr_id: ' . $drId . '.';
        }
        if (!$this->encounterReference) {
            $this->missingFields[] = 'Encounter Reference — kirim Encounter ke Satu Sehat dulu (modul kirim-encounter).';
        }
    }
    ...
    public function cariFaskes(): void
    {
        ...
        $this->dispatch('toast', type: $count > 0 ? 'success' : 'warning', message: $count > 0 ? "Ditemukan {$count} faskes tujuan." : 'Tidak ada faskes yang cocok.');
    }
    ...
    public function kirimRujukan(): void
    {
        ...
        try {
            ...
            $this->dispatch('toast', type: 'warning', message: 'Rujukan dibuat tapi gagal simpan state lokal: ' . $e->getMessage());
            return;
        }

        $this->dispatch('toast', type: 'success', message: 'Rujukan berhasil dibuat: ' . ($this->rujukanResult['noRujukan'] ?? '-'));
    }
    ...
    private function restoreState(): void
    {
        ...
        // Restore step: pakai yang tersimpan, fallback derive dari state.
        $savedStep = (int) ($saved['step'] ?? 0);
        if ($savedStep >= 1 && $savedStep <= 4) {
            $this->step = $savedStep;
        } elseif ($this->rujukanResult) {
            $this->step = 4;
        } elseif ($this->faskesTerpilih) {
            $this->step = 3;
        } elseif ($this->kriteriaSource === 'server') {
            $this->step = 2;
        } else {
            $this->step = 1;
        }
    ...
    private function buildKriteriaPayload(): array
    {
        return array_map(function ($it) {
            $answer = $it['type'] === 'boolean' ? ['valueBoolean' => (bool) $it['answer']] : ['valueString' => (string) $it['answer']];
            return [
        ...
        <div class="flex items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="text-[10px] px-1.5 py-0.5 rounded bg-indigo-200/60 dark:bg-indigo-800/40 text-indigo-700 dark:text-indigo-300 uppercase tracking-wider">Sisrute
                        · Satu Sehat</span>
                </div>
                <p class="mt-1 text-xs text-indigo-700/80 dark:text-indigo-300/70">
                    Cari faskes tujuan berdasarkan diagnosa + kompetensi, lalu kirim rujukan lengkap (Sisrute + Satu
                    Sehat).
                </p>
                @if ($rujukanResult)
                    <div
                        class="mt-2 inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-emerald-100 dark:bg-emerald-900/30 text-xs text-emerald-700 dark:text-emerald-300">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        Rujukan sudah dibuat: <span
                            class="font-mono font-bold">{{ $rujukanResult['noRujukan'] ?? '-' }}</span>
                    </div>
                @elseif (!empty($missingFields))
                    <div
                        class="mt-2 inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-amber-100 dark:bg-amber-900/30 text-xs text-amber-700 dark:text-amber-300">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            stroke-width="2">
            ...
            <div class="flex-1 overflow-y-auto px-6 py-5 bg-gray-50/70 dark:bg-gray-950/20 space-y-5">
                ...
                @php
                    $guide = [
                        [
                            'key' => 'A',
                            'title' => 'A. Data Rujukan (langkah 1)',
                            'items' => [
                                [
                                    'n' => '1',
                                    'head' => 'Tgl Rencana Kunjungan',
                                    'body' => 'Default = besok. Tanggal pasien datang ke RS tujuan. Ubah kalau perlu.',
                                ],
                                [
                                    'n' => '2',
                                    'head' => 'Diagnosa (ICD-10)',
                                    'body' =>
                                        'Otomatis terisi dari diagnosis pertama di EMR. Memicu auto fetch Kriteria + Jejaring Wilayah saat diubah.',
                                ],
                                [
                                    'n' => '3',
                                    'head' => 'Kode Spesialis',
                                    'body' =>
                                        'Otomatis dari kdpolibpjs pasien saat ini. User bisa override kalau rujuk ke spesialis berbeda. Dipakai untuk field "Spesialis/Sub Spesialis" di GetFaskesRujukan.',
                                ],
                                [
                                    'n' => '4',
                                    'head' => 'Poli Rujukan',
                                    'body' =>
                                        'Input manual kode poli BPJS 3-digit di faskes tujuan (mis. 005). Kalau kosong, fallback pakai Kode Spesialis.',
                                ],
                                [
                                    'n' => '5',
                                    'head' => 'Kode Sarana (opsional)',
                                    'body' =>
                                        'Parameter GetFaskesRujukan untuk filter faskes berdasarkan sarana khusus (mis. alat CT scan, MRI). Kosongkan kalau tidak perlu.',
                                ],
                                [
                                    'n' => '6',
                                    'head' => 'Jenis & Tipe Rujukan',
                                    'body' => 'Jenis: 2=RJ (default), 1=RI. Tipe: 0=Penuh, 1=Partial, 2=Rujuk Balik.',
                                ],
                            ],
                        ],
                        [
                            'key' => 'B',
                            'title' => 'B. Data Satu Sehat (auto)',
                            'items' => [
                                [
                                    'n' => '1',
                                    'head' => 'Kode Faskes Satu Sehat',
                                    'body' =>
                                        'Dari env SATUSEHAT_ORGANIZATION_ID — kode Satu Sehat RS sendiri. Tetap per instance RS.',
                                ],
                                [
                                    'n' => '2',
                                    'head' => 'ID Pasien Satu Sehat',
                                    'body' =>
                                        'Dari rsmst_pasiens.patient_uuid. Kalau kosong — sync pasien lewat modul Satu Sehat Patient dulu.',
                                ],
                                [
                                    'n' => '3',
                                    'head' => 'Kd Dokter Satu Sehat',
                                    'body' =>
                                        'Dari rsmst_doctors.dr_uuid untuk dokter DPJP. Kalau kosong — isi dulu di master dokter.',
                                ],
                                [
                                    'n' => '4',
                                    'head' => 'Encounter Reference',
                                    'body' =>
                                        'Dari dataRJ.satuSehat.encounterId. Kalau kosong — kirim Encounter Satu Sehat dulu (modul kirim-encounter di Satu Sehat RJ).',
                                ],
                            ],
                        ],
                        [
                            'key' => 'C',
                            'title' => 'C. Kriteria Rujukan + Wilayah (langkah 2)',
                            'items' => [
                                [
                                    'n' => '1',
                                    'head' => 'Klik tombol "⬇ Ambil Kriteria"',
                                    'body' =>
                                        'Trigger manual — sistem POST ke GET /Rujukan/GetKriteriaRujukan dengan diagnosa + kode faskes RS kita. Server balas: daftar pertanyaan kriteria (dinamis per diagnosa) + rekomendasi jejaring wilayah.',
                                ],
                                [
                                    'n' => '2',
                                    'head' => 'Fallback kalau belum dimuat',
                                    'body' =>
                                        'Sebelum user klik tombol, Section C tampil 3 kriteria default (3216 Terapy/Pengobatan boolean, 3215 Tindakan Medis ICD-9 string, 3214 Upaya Diagnosis boolean) sebagai fallback. Badge "⚠ Belum dimuat" di header.',
                                ],
                                [
                                    'n' => '3',
                                    'head' => 'Muat Ulang saat ganti diagnosa',
                                    'body' =>
                                        'Setelah kriteria pernah dimuat, ubah diagnosa di Section A → klik "🔄 Muat Ulang" supaya kriteria refetch sesuai diagnosa baru (auto-refresh tidak aktif — biar user kontrol timing).',
                                ],
                                [
                                    'n' => '4',
                                    'head' => 'Isi jawaban kriteria',
                                    'body' =>
                                        'Boolean = toggle Ya/Tidak. String = input kode (biasanya ICD-9 untuk Tindakan Medis). Field boleh kosong — tidak semua pertanyaan wajib.',
                                ],
                                [
                                    'n' => '5',
                                    'head' => 'Jejaring Wilayah (auto-fill)',
                                    'body' =>
                                        'Kode & nama Provinsi/Kabupaten auto-fill dari response server saat Ambil Kriteria. Boleh override kalau mau rujuk ke wilayah lain.',
                                ],
                            ],
                        ],
                        [
                            'key' => 'D',
                            'title' => 'D. Cari & Pilih Faskes (langkah 3)',
                            'items' => [
                                [
                                    'n' => '1',
                                    'head' => 'Klik "Cari Faskes"',
                                    'body' =>
                                        'POST /Rujukan/GetFaskesRujukan ke VClaim-Sisrute dengan diagnosa + kompetensi + sarana + kriteria + wilayah + encounter.',
                                ],
                                [
                                    'n' => '2',
                                    'head' => 'Tabel hasil',
                                    'body' =>
                                        'Per baris tampilkan: Nama PPK, Kode PPK, Kelas, Kota, Jadwal Praktek Spesialis, Jarak (meter), Kapasitas %.',
                                ],
                                [
                                    'n' => '3',
                                    'head' => 'Pilih 1 Faskes',
                                    'body' =>
                                        'Klik tombol "Pilih" di baris. kdppk + kodeFaskesSatuSehat tujuan tersimpan. Card konfirmasi + tombol Kirim muncul.',
                                ],
                                [
                                    'n' => '4',
                                    'head' => 'Klik "Kirim Rujukan"',
                                    'body' =>
                                        'POST /Rujukan/Insert (langkah 4). VClaim-Sisrute teruskan ke Satu Sehat. Output: noRujukan BPJS + noRujukanSatuSehat + serviceRequestId FHIR.',
                                ],
                            ],
                        ],
                        [
                            'key' => 'E',
                            'title' => 'E. Hasil & Persistensi (langkah 4)',
                            'items' => [
                                [
                                    'n' => '1',
                                    'head' => 'noRujukan BPJS',
                                    'body' =>
                                        'Nomor rujukan resmi BPJS, dipakai pasien saat datang ke RS tujuan (dicetak bersama SEP).',
                                ],
                                [
                                    'n' => '2',
                                    'head' => 'noRujukanSatuSehat',
                                    'body' => 'Nomor rujukan versi Satu Sehat Kemenkes.',
                                ],
                                [
                                    'n' => '3',
                                    'head' => 'serviceRequestId',
                                    'body' =>
                                        'FHIR ServiceRequest ID di Satu Sehat — reference ke resource rujukan di platform Kemenkes.',
                                ],
                                [
                                    'n' => '4',
                                    'head' => 'Detail lengkap',
                                    'body' =>
                                        'Panel hasil tampilkan: asal rujukan (RS kita), tujuan rujukan, poli tujuan, diagnosa, data peserta BPJS, tgl rujukan.',
                                ],
                                [
                                    'n' => '5',
                                    'head' => 'Persistensi',
                                    'body' =>
                                        'Hasil otomatis disimpan di rstxn_rjhdrs.datadaftarpolirj_json pada key rujukanKompetensi. Buka modal lagi → state form + hasil otomatis ter-restore.',
                                ],
                            ],
                        ],
                        [
                            ...
                            'items' => [
                                [
                                    'n' => '—',
                                    'head' => 'Scope RJ (Rawat Jalan)',
                                    'body' =>
                                        'Flow ini khusus RJ antar-faskes lewat gateway VClaim-Sisrute BPJS. UGD & Inap langsung ke Satu Sehat FHIR (trait lain, belum dibuat).',
                                ],
                                [
                                    'n' => '—',
                                    'head' => 'Alur 1 kali POST',
                                    'body' =>
                                        'VClaim-Sisrute = orkestrator gabungan. Kita POST ke BPJS, BPJS teruskan ke Satu Sehat Rujukan. Respons balik dapat kedua nomor rujukan (BPJS + Satu Sehat).',
                                ],
                                [
                                    'n' => '—',
                                    'head' => 'Prasyarat',
                                    'body' =>
                                        'SEP, Encounter Satu Sehat, patient_uuid, dr_uuid. Kalau salah satu belum ada, tombol akan gagal (tampil di kotak merah atas).',
                                ],
                                [
                                    'n' => '—',
                                    'head' => 'Env base URL',
                                    'body' =>
                                        'Set SISRUTE_URL = "https://apijkn-dev.bpjs-kesehatan.go.id/vclaim-sisrute-rest" di .env. Credential VClaim existing dipakai sebagai fallback.',
                                ],
                                [
                                    'n' => '—',
                                    'head' => 'Delete belum ada',
                                    'body' =>
                                        'Fitur hapus rujukan belum diimplementasi (stub 501). Pastikan faskes tujuan & data benar sebelum kirim.',
                                ],
                            ],
                        ],
                    ];
                ...
                @if (!empty($missingFields))
                    <div
                        class="p-4 rounded-xl border-2 border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-900/20">
                ...
                {{-- STEP INDICATOR — 4 langkah sesuai PDF BPJS (Kriteria → Faskes → Insert) --}}
                <div class="bg-white border border-gray-200 rounded-xl p-4 dark:bg-gray-900 dark:border-gray-700">
                        ...
                        @foreach ($steps as $n => $label)
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                <div @class([
                                    ...
                                    'text-indigo-700 dark:text-indigo-300' => $state === 'current',
                                    'text-gray-500' => $state === 'locked',
                                ])>{{ $label }}</div>
                            </div>
                            @if ($n < 4)
                                <div @class([
                                    'flex-1 h-px min-w-[20px]',
                                    'bg-emerald-500' => $step > $n,
                                    'bg-gray-200 dark:bg-gray-700' => $step <= $n,
                                ])></div>
                            @endif
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <div class="space-y-5 min-w-0">
                        ...
                        {{-- A. DATA RUJUKAN --}}
                        <div
                            class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <x-text-input wire:model="kodeDiagnosa" placeholder="mis. I10"
                                    ...
                                    @if (!empty($dataRJ['diagnosis'] ?? []))
                                        <p class="mt-1 text-[11px] text-gray-500">
                                            ...
                                            @foreach ($dataRJ['diagnosis'] as $d)
                                                <span
                                                    class="font-mono text-gray-700 dark:text-gray-300">{{ $d['icdX'] ?? '-' }}</span>{{ !$loop->last ? ', ' : '' }}
                                            @endforeach
                                            — bisa diedit manual bila perlu penyesuaian.
                                        </p>
                                    @else
                                        <p class="mt-1 text-[11px] text-amber-600 dark:text-amber-400">
                                            Belum ada diagnosa di EMR. Isi manual (ICD-10, mis. I10).
                                        </p>
                                    @endif
                                </div>
                                <div>
                                    <x-input-label value="Kode Spesialis *" />
                                    <x-text-input wire:model="kodeSpesialis" placeholder="mis. 095"
                                        class="mt-1 w-full" />
                                    <p class="mt-1 text-[11px] text-gray-500">Ambil dari referensi VClaim (API
                                        bridging).</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <x-text-input wire:model="poliRujukan" placeholder="mis. 005" maxlength="10"
                                        class="mt-1 w-full font-mono" />
                                    <p class="mt-1 text-[11px] text-gray-500">
                                        Kode poli BPJS standar nasional (3 digit) di faskes tujuan. Kalau kosong,
                                        fallback pakai Kode Spesialis.
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <x-textarea wire:model="patientInstruction" rows="2"
                                        class="mt-1 w-full"></x-textarea>
                                </div>
                                <div>
                                    <x-input-label value="Keterangan Rujukan" />
                                    <x-textarea wire:model="keteranganRujukan" rows="2"
                                        class="mt-1 w-full"></x-textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-900 dark:border-gray-700">
                            <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-700">
                                <h3 class="font-semibold text-gray-800 dark:text-gray-100">B. Data Satu Sehat (auto
                                    dari
                                    master)</h3>
                            </div>
                            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {{-- ACTION BAR: step 1 → 2 (trigger GetKriteriaRujukan) --}}
                        @if ($step === 1)
                            <div
                                class="rounded-xl border-2 border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20 p-4">
                                <div class="flex items-center justify-between gap-4 flex-wrap">
                                    <div class="min-w-0">
                                        <div class="text-sm font-semibold text-indigo-800 dark:text-indigo-200">Langkah
                                            1 — Lanjut ke Kriteria Rujukan</div>
                                        <div class="text-xs text-indigo-700/80 dark:text-indigo-300/80 mt-0.5">
                                            Tombol ini POST ke <span class="font-mono">GET
                                                /Rujukan/GetKriteriaRujukan</span> dengan diagnosa + kode faskes. Server
                                            balas: daftar pertanyaan kriteria + jejaring wilayah.
                                        </div>
                                    </div>
                                    <x-primary-button type="button" wire:click="muatKriteria"
                                        wire:loading.attr="disabled" wire:target="muatKriteria"
                                        class="!bg-indigo-600 hover:!bg-indigo-700 shrink-0">
                                        <span wire:loading.remove wire:target="muatKriteria">⬇ Ambil Kriteria</span>
                    ...
                    <div class="lg:sticky lg:top-0 lg:self-start">
                        <div
                            class="bg-white border border-indigo-200 shadow-sm rounded-xl dark:bg-gray-900 dark:border-indigo-700/40">
                            <div class="px-5 py-3 border-b border-indigo-100 dark:border-indigo-900/40">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex items-center justify-center w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/30">
                                    ...
                                    <div>
                                        <div class="font-semibold text-gray-800 dark:text-gray-100">Cara Pakai — Alur
                                            Rujukan Kompetensi</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">Sisrute = orkestrator
                                            gabungan BPJS + Kemenkes + Satu Sehat.</div>
                                    </div>
                                </div>
                            </div>
                                ...
                                @foreach ($guide as $g)
                                    <div x-data="{ sec: '{{ $g['key'] }}' }">
                                        ...
                                        <div x-show="activeSec === sec"
                                            ...
                                            x-transition:leave-end="opacity-0 -translate-y-2" class="pb-3 space-y-2"
                                            style="display: none;">
                                            @foreach ($g['items'] as $item)
                                                <div
                                                    class="flex items-start gap-3 p-3 border border-gray-100 rounded-lg bg-gray-50 dark:bg-gray-800/50 dark:border-gray-700">
                                                    <div
                                                        class="flex items-center justify-center min-w-[28px] h-7 px-1.5 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold shrink-0 dark:bg-indigo-900/30 dark:text-indigo-300">
                                                        {{ $item['n'] }}
                                                    </div>
                ...
                {{-- C. KRITERIA RUJUKAN (full-width, di bawah B) — hanya muncul setelah GetKriteriaRujukan sukses --}}
                @if ($step >= 2)
                    <div
                        class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-900 dark:border-gray-700">
                        <div
                            class="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <h3 class="font-semibold text-gray-800 dark:text-gray-100">C. Kriteria Rujukan</h3>
                                <p class="text-xs text-gray-500 mt-1">
                                    @if ($kriteriaSource === 'server')
                                        <span class="text-emerald-600 dark:text-emerald-400 font-semibold">✓ Dari
                                            Server</span>
                                        — kriteria &amp; jejaring wilayah sudah dimuat dari Satu Sehat Rujukan.
                                        Klik "Muat Ulang" kalau diagnosa diubah.
                                    @else
                                        <span class="text-amber-600 dark:text-amber-400 font-semibold">⚠ Belum
                                            dimuat</span>
                        ...
                        <div class="p-5 space-y-3">
                            @foreach ($kriteriaItems as $idx => $item)
                                <div class="flex items-start gap-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                                    <div class="flex-1 min-w-0">
                                        <div class="text-xs text-gray-500 font-mono">linkId: {{ $item['linkId'] }}
                                        </div>
                                        <div class="font-semibold text-gray-800 dark:text-gray-100">
                                            {{ $item['text'] }}
                                        </div>
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <h4 class="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">Jejaring
                                    Wilayah</h4>
                ...
                @endif {{-- end @if ($step >= 2) Section C --}}

                {{-- ACTION BAR: step 2 → 3 (trigger GetFaskesRujukan) --}}
                @if ($step === 2)
                    <div
                        class="rounded-xl border-2 border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20 p-4">
                        <div class="flex items-center justify-between gap-4 flex-wrap">
                            <div class="min-w-0">
                                <div class="text-sm font-semibold text-indigo-800 dark:text-indigo-200">Langkah 2 —
                                    Lanjut ke Cari Faskes Tujuan</div>
                                <div class="text-xs text-indigo-700/80 dark:text-indigo-300/80 mt-0.5">
                                    Isi kriteria + wilayah di atas, lalu klik. Tombol ini POST ke <span
                                        class="font-mono">/Rujukan/GetFaskesRujukan</span> — server balas daftar RS
                                    yang bisa dirujuk.
                                </div>
                            </div>
                            <x-primary-button type="button" wire:click="cariFaskes" wire:loading.attr="disabled"
                                wire:target="cariFaskes" class="!bg-indigo-600 hover:!bg-indigo-700 shrink-0">
                ...
                {{-- D. AKSI + HASIL FASKES (full-width) — hanya muncul setelah GetFaskesRujukan sukses --}}
                @if ($step >= 3)
                    <div
                        class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-900 dark:border-gray-700">
                        <div
                            class="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between gap-3">
                            <h3 class="font-semibold text-gray-800 dark:text-gray-100">D. Cari Faskes Tujuan</h3>
                            <x-primary-button type="button" wire:click="cariFaskes" wire:loading.attr="disabled"
                                wire:target="cariFaskes" class="!bg-gray-500 hover:!bg-gray-600">
                                <span wire:loading.remove wire:target="cariFaskes">🔄 Cari Ulang</span>
                                <span wire:loading wire:target="cariFaskes"><x-loading /> Mencari…</span>
                            </x-primary-button>
                        </div>
                                        ...
                                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach ($faskesList as $idx => $fk)
                                                ...
                                                <tr
                                                    ...
                                                    <td class="px-3 py-2 font-mono text-xs">{{ $fk['kdppk'] ?? '-' }}
                                                    </td>
                                                    <td class="px-3 py-2">{{ $fk['kelas'] ?? '-' }}</td>
                                                    <td class="px-3 py-2">{{ $fk['nmkc'] ?? '-' }}</td>
                                                    <td
                                                        class="px-3 py-2 text-xs max-w-[240px] text-gray-600 dark:text-gray-400">
                                                        {{ $fk['jadwal'] ?? '-' }}
                                                    </td>
                                                    <td class="px-3 py-2 text-right font-mono">
                                                        {{ number_format($fk['distance'] ?? 0, 0) }}</td>
                                                    <td class="px-3 py-2 text-right">{{ $fk['persentase'] ?? 0 }}%
                                                    </td>
                ...
                @if ($rujukanResult)
                    <div
                        ...
                        <div
                            class="px-5 py-3 border-b border-brand/20 dark:border-brand-lime/20 flex items-center gap-2">
                        ...
                        <div class="p-5 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div class="p-3 rounded-lg bg-brand/10 dark:bg-brand-lime/10">
                                    <div
                                        class="text-[10px] uppercase tracking-wider text-brand dark:text-brand-lime font-semibold">
                                        No Rujukan BPJS</div>
                                ...
                                <div class="p-3 rounded-lg bg-indigo-50 dark:bg-indigo-900/20">
                                    <div
                                        class="text-[10px] uppercase tracking-wider text-indigo-600 dark:text-indigo-400 font-semibold">
                                ...
                                <div class="p-3 rounded-lg bg-purple-50 dark:bg-purple-900/20">
                                    <div
                                        class="text-[10px] uppercase tracking-wider text-purple-600 dark:text-purple-400 font-semibold">
