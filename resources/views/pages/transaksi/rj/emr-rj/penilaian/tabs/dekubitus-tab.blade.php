...
<div class="space-y-4">
    ...
    @if (collect($penilaian['dekubitus'] ?? [])->filter(fn($r) => filled(data_get($r, 'tglPenilaian')))->isNotEmpty())
            ...
            <div class="overflow-x-auto rounded-lg border border-hairline dark:border-gray-700">
                <table class="w-full text-sm text-left text-muted dark:text-gray-300">
                    ...
                    <tbody class="divide-y divide-hairline-soft dark:divide-gray-700">
                        @foreach (array_reverse(array_filter($penilaian['dekubitus'] ?? [], fn($r) => filled(data_get($r, 'tglPenilaian'))), true) as $i => $row)
                            @php
                            ...
                            <tr class="{{ $rowBg }}">
                                ...
                                @if (!$isFormLocked)
                                    <td class="px-3 py-2">
                                        <x-hapus-button wire:click="removeAssessmentDekubitus({{ $i }})" confirm="Hapus data dekubitus ini?" />
                                    </td>
                                @endif
                            </tr>
                        @endforeach
