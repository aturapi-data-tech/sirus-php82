...
new class extends Component {
    ...
    /* ===============================
     ...
     | Cabut TTD SATU bagian di apotekHdr[$idx]; isian butir dipertahankan. Dicatat ke audit log.
     =============================== */
    public function bukaKunciTelaah(string $bagian): void
    {
        ...
        try {
            DB::transaction(function () use ($bagian, $label) {
                ...
                $idx = $this->ensureApotekHdrIndex($fresh, $this->slsNo);
                if (empty($fresh['apotekHdr'][$idx][$bagian]['penanggungJawab'])) {
                    throw new \RuntimeException($label . ' belum ditandatangani.');
                }
                $pj = $fresh['apotekHdr'][$idx][$bagian]['penanggungJawab'];
                unset($fresh['apotekHdr'][$idx][$bagian]['penanggungJawab']);
                $this->updateJsonRI($this->riHdrNo, $fresh);
                $this->gantiStateDariDb($fresh, $idx, $bagian === 'telaahResep' ? 'telaahObat' : 'telaahResep');
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Buka kunci ' . $label . ' apotek (resep ' . $this->slsNo . ') — TTD ' . ($pj['userLog'] ?? '-') . ' (' . ($pj['userLogDate'] ?? '-') . ') dicabut oleh ' . (auth()->user()->myuser_name ?? auth()->user()->name ?? '-'), 'MR');
            });
            $this->incrementVersion('ri-resep-modal-telaah');
     ...
     * lain (di apotekHdr[$idx]) yang baru diubah di layar tapi belum disimpan.
     */
    ...
    <x-modal name="ri-resep-telaah" size="full" height="full" focusable>
        <div wire:key="{{ $this->renderKey('ri-resep-modal-telaah', [$slsNo ?? 'new']) }}">
            ...
            @else
                ...
                @php
                    $ttdResep = $apotek['telaahResep']['penanggungJawab'] ?? null;
                    $ttdObat = $apotek['telaahObat']['penanggungJawab'] ?? null;
                @endphp
                <div
