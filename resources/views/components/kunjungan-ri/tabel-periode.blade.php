        ...
        <table class="min-w-full text-sm">
            <thead class="bg-surface-card dark:bg-gray-800">
                <tr class="text-xs font-semibold tracking-wide text-muted uppercase dark:text-gray-300">
                    ...
                    <th colspan="3" class="px-3 pt-2 pb-1 text-center text-blue-700 border-b border-hairline dark:text-blue-300 dark:border-gray-700">Komponen Hitungan</th>
                ...
                <tr class="text-xs font-semibold tracking-wide text-muted uppercase dark:text-gray-300">
                    ...
                    <th class="px-3 py-2 text-right text-blue-700 dark:text-blue-300" title="Σ (exit_date − entry_date) pasien keluar, dibebankan ke periode tanggal pulang">Σ Lama Dirawat</th>
            ...
            <tbody>
                @foreach ($rows as $row)
                    <tr class="border-t border-hairline-soft dark:border-gray-800 hover:bg-surface-soft dark:hover:bg-gray-800/50 {{ $row['total'] === 0 ? 'opacity-50' : '' }}">
                        ...
                        <td class="px-2 py-2.5 text-right tabular-nums text-purple-700 dark:text-purple-300" title="{{ $rumusSel($row, 'alos') }}">{{ $row['total'] > 0 ? $row['alos'] . ' hr' : '—' }}</td>
            ...
            <tfoot class="bg-surface-soft dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-600">
                <tr class="text-sm font-bold text-ink dark:text-gray-100">
                    ...
                    <td class="px-2 py-3 text-right tabular-nums text-purple-800 dark:text-purple-200" title="{{ $rumusSel($totals, 'alos') }}">{{ $totals['total'] > 0 ? $totals['alos'] . ' hr' : '—' }}</td>
    ...
    <div class="px-4 py-2 text-[10px] text-muted dark:text-gray-500 border-t border-hairline-soft dark:border-gray-800">
        *) Pasien unik global. Arahkan kursor ke angka BOR / ALOS / TOI / BTO untuk melihat rumus berikut angkanya.
        ...
        @if ((int) $kapasitasTT !== (int) $defaultKapasitasTT)
            <span class="text-amber-600 dark:text-amber-400">(diubah dari bawaan DB {{ $angka($defaultKapasitasTT) }})</span>
        @endif.
