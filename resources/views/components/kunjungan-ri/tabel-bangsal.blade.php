...
@php
    ...
    $rumusSel = function (array $row, string $indikator) use ($hariPeriode, $angka) {
        ...
        $keluar = $angka($row['pasien_bangsal']);
    ...
    $jumlah = [
        ...
        'pasien_bangsal' => array_sum(array_column($rowsDihitung, 'pasien_bangsal')),
    ...
    $jumlah['alos'] = $jumlah['pasien_bangsal'] > 0 ? round($jumlah['total_los'] / $jumlah['pasien_bangsal'], 1) : null;
    $jumlah['bto'] = ($jumlah['tt'] > 0 && $hariPeriode > 0) ? round($jumlah['pasien_bangsal'] / $jumlah['tt'], 2) : null;
    $jumlah['toi'] = ($jumlah['pasien_bangsal'] > 0 && $jumlah['hari_tersedia'] > 0) ? round(($jumlah['hari_tersedia'] - $jumlah['total_los']) / $jumlah['pasien_bangsal'], 1) : null;
...
<div class="mt-4 bg-canvas border border-hairline shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        ...
        <table class="min-w-full text-sm">
            <thead class="sticky top-0 z-10 bg-surface-card dark:bg-gray-800">
                <tr class="text-xs font-semibold tracking-wide text-muted uppercase dark:text-gray-300">
                    ...
                    <th class="px-3 py-3 text-right text-blue-700 dark:text-blue-300" title="Pasien yang DIRAWAT di bangsal ini menurut riwayat kamar — pulang dari sini maupun dipindahkan ke bangsal lain. Penyebut ALOS / BTO / TOI.">Pasien Dirawat</th>
                    <th class="px-3 py-3 text-right" title="Pasien yang PULANG dari bangsal ini (kamar terakhir). Penyebut NDR / GDR.">Pulang Dari Sini</th>
                    <th class="px-2 py-3 text-right text-rose-700 dark:text-rose-400" title="Pasien yang pulang dari bangsal ini tetapi TIDAK ikut hitungan BOR / ALOS / BTO / TOI: status bukan P, lama dirawat negatif, tanpa tanggal masuk, atau di atas batas wajar">Dikeluarkan</th>
                    <th class="px-3 py-3 text-right text-blue-700 dark:text-blue-300" title="Σ (selesai − mulai) tiap segmen kamar di bangsal ini menurut riwayat kamar, tanpa data janggal yang dikeluarkan">Σ Hari Rawat</th>
                    <th class="px-2 py-3 text-right text-amber-700 dark:text-amber-400" title="Pasien yang pulang dari bangsal ini dengan lama dirawat (seluruh kunjungan) di bawah 1 hari">LOS &lt; 1 hr</th>
            ...
            <tbody>
                @forelse ($rows as $nomor => $row)
                    ...
                    <tr class="border-t border-hairline-soft dark:border-gray-800 hover:bg-surface-soft dark:hover:bg-gray-800/50 {{ $dihitung ? '' : 'bg-surface-soft/60 dark:bg-gray-800/40' }}">
                        ...
                        <td class="px-3 py-2.5 text-right tabular-nums font-semibold text-blue-700 dark:text-blue-300">{{ $angka($row['pasien_bangsal']) }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="17" class="px-6 py-12">
            ...
            @if (count($rows) > 0)
                <tfoot class="bg-surface-soft dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-600">
                    <tr class="text-sm font-bold text-ink dark:text-gray-100">
                        ...
                        <td class="px-3 py-3 text-right tabular-nums text-blue-800 dark:text-blue-200" title="Pasien yang pindah bangsal terhitung di tiap bangsal yang disinggahinya">{{ $angka($jumlah['pasien_bangsal']) }}</td>
    ...
    <div class="px-4 py-2 text-[10px] leading-snug text-muted dark:text-gray-500 border-t border-hairline-soft dark:border-gray-800">
        ...
        <strong>Hari rawat</strong> dihitung dari riwayat kamar: tiap hari jatuh ke bangsal tempat pasien benar-benar dirawat, termasuk bangsal transit (ICU, perinatologi).
        <strong>Pasien Dirawat</strong> = pulang dari bangsal ini + dipindahkan ke bangsal lain, jadi jumlahnya bisa melebihi total pasien keluar RS.
