...
| Skill | Cakupan | Baca saat |
|---|---|---|
| **modul-dokumen** | Membuat/mem-port modul dokumen bertanda tangan (consent, surat keterangan, laporan, Akhir Hayat): kartu+tombol→modal, Draft→TTD→Kunci→Lihat/Cetak, role Gate terpusat, penanda tab, viewer Rekam Medis, porting RI⇄UGD⇄RJ, kode formulir RM (RM-KK.NN). | Membuat form dokumen baru, memasang di jalur lain, atau viewer rekam-medisnya. |
...
| **apotek-online** | Modul Apotek Online BPJS (`apotek-rest`): klaim obat PRB/kronis/kemo. `ApotekTrait` 18 endpoint + 4 layar, alur klaim, jebakan payload (KDOBAT vs KDOBT, SEP alfanumerik, DELETE json, porsi kronis qty_kronis bukan qty resep), node JSON `apotekOnline`, LOV `kode_dpho`. | Sebelum menyentuh kode Apotek Online (`ApotekTrait`, `/apotek-online`, laporan klaim, LOV DPHO), atau saat kiriman ditolak BPJS. Docs web: `/panduan-dev/apotek-online` |
