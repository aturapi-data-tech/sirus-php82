...
1. Task `referral-pre-request` → Task `request-referral-candidate` — contained QuestionnaireResponse **Q100** (kriteria; IGD = 5 pertanyaan GAWAT DARURAT linkId 000001–000005, tanpa validasi ICD) + **Q101** (jejaring wilayah, `valueCoding` WAJIB — `valueString` menghasilkan 0 kandidat); input: primary-diagnosis ICD-10, management procedure SNOMED (IGD `385868005`, ranap `737481003`, RJ `737492002` — sesuai Postman V30062026), Kelompok Layanan (Playbook Lampiran 4).
   **Q100 ranap: linkId DAN teks item DINAMIS per ICD-10, sama seperti SISRUTE.** Sumbernya `contained` Questionnaire "Kriteria Rujukan" di balasan Task Pra Permintaan (K56.7: Terapi `67627`, Tindakan Medis `42718`, Upaya Diagnosis `17809`; teks "Terapi", bukan "Terapi/Pengobatan"). linkId contoh Postman `3216/3215/3214` hanya sah untuk I61.9. Salah linkId dibalas **HTTP 201 berisi OperationOutcome** (`linkId … tidak valid, linkId valid …: <n>`), bukan 4xx — sampai 23/09/26 panel mengira kandidat "belum keluar" dan tujuan ranap tak pernah memunculkan kandidat, sedangkan IGD (linkId tetap `000001–000005`) normal. Kini: `rujukanKriteriaRanapDariPraPermintaan()` + `rujukanOperationOutcomeGagal()`.
   **Sejak 23/09/26 (sore) Pra Permintaan jadi langkah tersendiri** — tombol *Ambil Kriteria dari SATUSEHAT* (`ambilKriteria()` di 3 panel FHIR) menampilkan Task ID Pra Permintaan dan merender pilihan kriteria DARI balasan server (ranap: radio sesuai item Questionnaire; IGD: pertanyaan grup `0` via `rujukanPertanyaanIgdDariPraPermintaan()`). Sebelum diambil, pilihan kriteria tidak tampil; *Cari Kandidat* tidak lagi mengulang Pra Permintaan. Kriteria terikat pasangan `jalur|diagnosa` (`kriteriaDimuatUntuk`) — ganti salah satunya mengosongkan kriteria. Alasan: layar mengikuti urutan Playbook, dan TC01–TC03 UAT Ranap/Darurat bisa dibuktikan dari layar, bukan hanya dari log.
